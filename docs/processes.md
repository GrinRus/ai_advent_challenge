...
- Для structured sync (`/api/llm/chat/sync`) покрывайте happy-path модульными/интеграционными тестами: проверка `BeanOutputConverter` схемы, строгого `responseFormat`, корректного маппинга usage/latency и fallback-сценариев (422 при нарушении JSON Schema, повторные попытки при 429/5xx).
...
## Наблюдаемость
- При включении structured sync логируйте latency и значения `promptTokens/completionTokens/totalTokens`, чтобы быть готовыми к интеграции метрик в последующих волнах.

...
### Backend (backend-mcp)
- [ ] Ввести интерфейс `PatchGenerator` и реализацию `ClaudeCliPatchGenerator`, которая собирает JSON-команду (`instructions`, `targetPaths`, `forbiddenPaths`, `contextSnippets`) и запускает `claude -p <payload> --output-format json --no-ansi` внутри workspace. Проверить, что возвращаемый JSON содержит `summary`, `diff` (unified), `files[]`, `risks[]`, `usage.promptTokens/completionTokens`; навесить валидации лимитов (`coding.max-*`) до и после вызова. → `backend-mcp/src/main/java/com/aiadvent/mcp/backend/coding/*`
- [ ] Добавить `ClaudeCliService`: поиск бинаря по `CLAUDE_CODE_BIN` или `$PATH`, валидация (`claude --version`), прокидка `ZHIPU_API_KEY`, конфигурация таймаутов (`CLAUDE_CODE_TIMEOUT`), запуск через `ProcessBuilder` с отдельными потоками stdout/stderr и редактированием ANSI (флаги `--no-ansi`). Вернуть типизированный ответ/ошибку (timeout, non-zero exit, stderr warning). → `backend-mcp/src/main/java/com/aiadvent/mcp/backend/coding`
- [ ] Зафиксировать работу с GLM (z.ai): вынести в свойства `coding.claude.base-url`, `coding.claude.model`, `coding.claude.api-key`, по умолчанию использовать `https://api.z.ai/api/anthropic` и модели `GLM-4.6/GLM-4.5/GLM-4.5-Air`, маппируя «balanced/fast/advanced» профили coding flow на соответствующие названия GLM. → `backend-mcp/src/main/java/com/aiadvent/mcp/backend/coding/CodingAssistantProperties.java`, `ClaudeCliService`
- [ ] Использовать `WorkspaceFileService` для подготовки контекста CLI: читать snippets в пределах `coding.max-context-bytes`, складывать промежуточный `claude-context.json`/`claude-prompt.md` во временную директорию workspace, очищать артефакты после завершения и логировать структуру промпта без содержимого. → `backend-mcp/src/main/java/com/aiadvent/mcp/backend/workspace/WorkspaceFileService.java`
- [ ] Перенастроить `CodingAssistantService` на новую реализацию генератора: внедрить `PatchGenerator`, добавить метрики `coding_patch_generation_duration`/`coding_claude_cli_fail_total`, логировать размеры prompt/diff, фильтровать бинарные блоки, обновлять `PatchRegistry` и флаг `requiresManualReview` при ошибках CLI или предупреждениях. Предусмотреть feature-flag для fallback на patch-plan. → `backend-mcp/src/main/java/com/aiadvent/mcp/backend/coding/CodingAssistantService.java`, `CodingAssistantProperties.java`, `PatchRegistry.java`
- [ ] Нормализовать usage/аннотации из CLI-ответа: маппинг `files[]` → `PatchAnnotations.files`, `risks[]` → `PatchAnnotations.risks`, отдельное поле `warnings` для dry-run ограничений; usage сохранять в `PatchUsage` и экспонировать в ответах MCP-инструментов/метриках. → `backend-mcp/src/main/java/com/aiadvent/mcp/backend/coding/*`
- [ ] Покрыть тестами: unit на `ClaudeCliService` (успех, stderr, timeout, смена провайдера и модели), unit на `ClaudeCliPatchGenerator` (некорректный JSON, превышение лимитов, бинарные diff'ы), интеграционный сценарий generate→review→apply с заглушкой CLI (скрипт, возвращающий детерминированный JSON). → `backend-mcp/src/test/java/com/aiadvent/mcp/backend/coding/*`

### Инфраструктура и конфигурация
- [ ] Добавить переменные `CLAUDE_CODE_BIN`, `CLAUDE_CODE_TIMEOUT`, `CODING_CLAUDE_ENABLED`, `CODING_CLAUDE_BASE_URL`, `CODING_CLAUDE_MODEL`, `ZHIPU_API_KEY` в `.env.example`, `application-coding.yaml` (по умолчанию `https://api.z.ai/api/anthropic`). → `.env.example`, `backend-mcp/src/main/resources/application-coding.yaml`
- [ ] Обновить `docker-compose.yml` и образ `coding-mcp`: установить Node.js 18+/npm, `npm install -g @anthropic-ai/claude-code`, смонтировать кеш `~/.npm`, прокинуть `ZHIPU_API_KEY`, добавить healthcheck `claude --version` и опцию для внешнего CLI volume. → `docker-compose.yml`, `backend-mcp/Dockerfile`
- [ ] Создать раздел в `docs/infra.md` (dev/prod): требования к Node/npm, установка CLI, управление ключами `ZHIPU_API_KEY`, проверка `claude --version`, настройка GLM endpoint (`https://api.z.ai/api/anthropic`, модели GLM-4.5/GLM-4.6), распространение обновлений, очистка кешей. → `docs/infra.md`

### Наблюдаемость и безопасность
- [ ] Добавить структурированное логирование генерации патча: фиксировать `promptBytes`, `contextBytes`, `diffBytes`, статус CLI (ok/timeout/error), маскировать stderr и usage перед выводом. → `backend-mcp/src/main/java/com/aiadvent/mcp/backend/coding/CodingAssistantService.java`
- [ ] Добавить конфиги feature-flag/ретраев: `coding.claude.enabled`, `coding.claude.max-retries`, стратегия fallback на patch-plan при деградации; документировать redaction stderr/usage и лимиты на повторные попытки. → `backend-mcp/src/main/java/com/aiadvent/mcp/backend/coding/CodingAssistantProperties.java`, `application-coding.yaml`
- [ ] Обновить `docs/guides/mcp-operators.md` и `docs/architecture/coding-assistant-rfc.md`: описать CLI-пайплайн, новые чек-листы диагностики (timeout, `claude --version`, Node/npm), управление ключами/пермишенами CLI, очистку temp-каталогов. → `docs/guides/mcp-operators.md`, `docs/architecture/coding-assistant-rfc.md`

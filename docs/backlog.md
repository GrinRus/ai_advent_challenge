...
- [ ] Подключить нативный Tree-sitter (JavaCPP/JNI): добавить загрузку `TSParser/TSLanguage`, заменить эвристику в `TreeSitterParser` на обход AST, поддержать флаг `github.rag.ast.native-enabled` и fallback.
- [ ] Расширить fixtures (`backend-mcp/src/test/resources/mini-repos/{java,kotlin,typescript,javascript,python,go}`) наследованием/перегрузками/implements, обновить тесты (`RepoRagIndexServiceMultiLanguageTest`, AST unit) на проверки FQN, docstring, edges, ошибок парсинга.

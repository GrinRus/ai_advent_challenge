...
- [ ] Обновить UI/DTO Flow каталога: карточка flow с описанием, формой ввода параметров, подсказками по выбору режима и отображением ожидаемых артефактов; убедиться, что `flow_ops.list_flows` отдаёт полное описание для LLM.

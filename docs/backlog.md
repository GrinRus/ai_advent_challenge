...
### Аналитика
- [ ] Исследовать поддержку `stream_options.include_usage` у провайдеров (OpenAI, Azure OpenAI, zhipu.ai): зафиксировать, где usage доступен «из коробки», где приходит только в финальном чанке, а где отсутствует.
- [ ] Оценить варианты кастомного токенайзера для fallback:
  - Java-библиотеки `jtokkit`, `gpt3-tokenizer-java`.
  - Обёртка над `tiktoken` через JNI/JEP или отдельный микросервис.
  - Собственный BPE/Unigram токенайзер (расчёт + кеширование).
- [ ] Составить сравнительную таблицу (точность, производительность, лицензия) и выбрать стратегию для production.
...
- [ ] Реализовать `TokenUsageEstimator` с fallback-сценарием: если провайдер не вернул usage, используем кастомный токенайзер.
- [ ] Имплементировать адаптеры для выбранного кастомного токенайзера, обеспечить конфигурируемость (вкл./выкл., выбор реализации).
- [ ] Интегрировать fallback в стриминговый конвейер (ZhiPu и другие провайдеры без usage), логировать источник значения (native vs. fallback).
- [ ] Добавить контрактные/юнит-тесты для оценок токенов (набор эталонных промтов, сравнение с эталонными значениями).

### Инфраструктура и документация
- [ ] Обновить `docs/infra.md` и `docs/processes.md`: описать использование `stream-usage`, поведение fallback токенайзера, ограничения по провайдерам.
- [ ] Подготовить ADR/раздел в `docs/releases` с выбором кастомного токенайзера и обоснованием.
- [ ] Добавить метрики и алерты для контроля расхождения между native usage и fallback (если оба доступны).

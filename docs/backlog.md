...
- [ ] Предусмотреть режим “без аутентификации”: backend принимает dev-only токен из `.env` (`PROFILE_BASIC_TOKEN`), передаваемый в заголовке `X-Profile-Auth`, и разрешает работу с профилями без OAuth/рольной модели (read/write в рамках одного токена). Требуется централизованный guard, который отклоняет запросы без токена и полностью выключает дев-режим, когда `PROFILE_DEV_ENABLED=false`. Задокументировать ограничения, предусмотреть флаг для отключения в проде и минимальные логи.  
...
- [ ] Ввести единый security guard для `/api/profile/**`, `/api/admin/**`, `/api/llm/**`, `/api/flows/**`, который валидирует `X-Profile-Key`, dev-токен и набор ролей до входа в контроллер, публикует структурные логи отказов и метрики (403/401), а также обеспечивает доступ к админ-эндпоинтам только для `admin`.
- [ ] Нормализовать `user_identity` (provider/externalId) при сохранении/удалении: хранить их в стандартизованном регистре, предотвращать дубли с разным написанием и добавить regression-тесты на attach/detach.
...
- [ ] Спроектировать базовую админ-панель (можно отдельным route) для управления ролями: таблица профилей, фильтры по namespace, действия “назначить роль”, “убрать роль”, просмотр истории. UI реализован, но ограничения по роли `admin` необходимо внедрить как на backend (`/api/admin/roles`), так и на frontend (скрывать раздел без прав).  
...
- [ ] Документировать и проверить dev-only режим: тесты подтверждают, что при включённом `PROFILE_BASIC_TOKEN` запросы без аутентификации принимаются, но роли/OAuth отключены; при отключенном флаге доступ запрещён. Добавить описание в `docs/infra.md` + негативные сценарии (нет токена, неверный токен, отключенный флаг).

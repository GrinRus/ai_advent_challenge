...
- [ ] Поддержать стратегии `OFF`/`LINEAR`/`PARENT_SYMBOL`/`CALL_GRAPH`: `LINEAR` — ±radius по индексу, `PARENT_SYMBOL` — все чанки с тем же `symbol_fqn`, `CALL_GRAPH` — вызванные/вызывающие символы (станет доступно после блока «AST-aware indexing» с `RepoRagSymbolService`). Для отсутствующих соседей логируем debug и идём дальше без ошибок. → `backend-mcp/src/main/java/com/aiadvent/mcp/backend/github/rag/postprocessing/NeighborChunkDocumentPostProcessor.java`, `RepoRagSymbolService.java`

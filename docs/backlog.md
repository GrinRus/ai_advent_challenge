...
- [ ] Реализовать `TreeSitterParser`/`AstNode` обёртку вокруг собраных грамматик (`treesitter/*`), поддерживающую Java/Kotlin/TS/JS/Python/Go (минимум) и готовую к расширению. Прогонять парсинг внутри `AstFileContextFactory` вместо `HeuristicAstExtractor`. Управление parser-ами возложить на `TreeSitterAnalyzer`: кешировать лес на файл, реиспользовать для chunker-а и graph-sync.
- [ ] Стандартизировать FQN в формате `package.Class#method(argTypes)`/`package.Class` для типов. Построить нормализованный DTO `SymbolNode` (uuid, fqn, kind, modifiers, signature, docstring, span, imports, parentId) и набор edge DTO (`CONTAINS`, `CALLS`, `IMPLEMENTS`, `READS_FIELD`, `USES_TYPE`). Обновить `AstSymbolMetadata`, `Chunk` и semantic chunker, чтобы они опирались на новую структуру.
- [ ] Подготовить regression-fixtures с наследованием, перегрузками, интерфейсами, docstring и разными модификаторами на каждом языке (`backend-mcp/src/test/resources/mini-repos`). Тест `RepoRagIndexServiceMultiLanguageTest` должен проверять корректность FQN, docstring, наборов edges, обработку ошибок парсинга и сравнивать ожидания по calls/implements.

### Графовая БД и синхронизация
- [ ] Зафиксировать выбор Neo4j: описать схему (`(:Repo {namespace})-[:CONTAINS]->(:File {path})-[:DECLARES]->(:Symbol {fqn,...})`, `CALLS`, `IMPLEMENTS`, `READS_FIELD`, `USES_TYPE`), индексы на `Symbol.fqn` и `File.path`, требования к деплою (Aura/Cluster), переменные `GITHUB_RAG_GRAPH_URI/USER/PASSWORD`.
- [ ] Создать `GraphSyncService`, который после chunking батчево отправляет `SymbolNode`/`Edge` в граф: `UNWIND $nodes ... MERGE`, `UNWIND $edges ... MERGE`. Вызывать сервис асинхронно из `RepoRagIndexService`, но блокировать завершение job, пока sync не завершён (или пока очередь не примет работу), чтобы `graphReadyAt` отражал фактическое состояние.
- [ ] Настроить инкрементальные апдейты: при удалении файла чистить узлы/ребра, при изменении — пересобирать только затронутые символы. Idempotent операции + логирование размера батчей, времени sync, числа добавленных/удалённых узлов. Добавить конфиг флага `github.rag.graph.legacy-table-enabled` для отключения старого `RepoRagSymbolGraphWriter`.

### MCP инструменты и использование графа
- [ ] Добавить инструменты `repo.code_graph_neighbors`, `repo.code_graph_path`, `repo.code_graph_definition`: вход — `namespace`, `symbolFqn`, `direction`, `relation`, `depth/limit`. Выход — JSON `{nodes:[{id,fqn,file,line,kind}], edges:[{from,to,relation}]}` + подсказки для UI. Контракты описать в `docs/tools.md`.
- [ ] Переписать `NeighborChunkDocumentPostProcessor` и `RepoRagSymbolService` на графовую БД: вместо таблицы `repo_rag_symbol_graph` исполнять подготовленные Cypher/GQL запросы и возвращать точные соседи. Фолбэком остаётся LINEAR/PARENT при `graphReady=false` или ошибке Neo4j; предупреждение писать в `warnings[]`.
...
- [ ] Добавить метрики `graph_nodes_total`, `graph_edges_total{relation}`, latency на записи/чтение, алерты на деградацию (`graph_sync_failure_total`, `graph_lookup_throttled_total`). Репортить `astReady`, `graphReady`, `graphSchemaVersion` в `repo.rag_index_status` (и MCP tool `repo.rag_index_status`).
- [ ] Написать миграционный план: как включать граф для новых namespace, как перегенерировать существующие, как откатываться на Postgres (feature-flag + queue). Описать чек-лист в `docs/processes.md`, диаграмму обновить в `docs/architecture/github-rag-modular.md`.

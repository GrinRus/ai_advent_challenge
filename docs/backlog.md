...
### Продукт и архитектура
- [ ] Обновить `docs/architecture/github-rag-indexing.md` и вынести отдельный раздел `docs/architecture/github-rag-modular.md` с описанием LEGO-подобных модулей (QueryTransformer, MultiQueryExpander, DocumentJoiner, DocumentPostProcessor, ContextualQueryAugmenter), схемой включения/отключения модулей и требованиями к latency/стоимости.
- [ ] Зафиксировать пользовательские сценарии (агенты, flows, операторы), матрицу параметров (`useCompression`, `translateTo`, `multiQueryK`, `allowEmptyContext`, `filterExpression`) и политики безопасности (ограничения на languages, максимальное число подзапросов). → `docs/architecture/github-rag-modular.md`, `docs/guides/mcp-operators.md`

### Backend MCP (GitHub) — RAG pipeline
- [ ] Добавить pre-retrieval слой: собрать `CompressionQueryTransformer`, `RewriteQueryTransformer`, `TranslationQueryTransformer` с отдельным `ChatClient.Builder` (temperature=0) и конфигурируемыми промптами; расширить `GitHubRagProperties`/`application-github.yaml`/`.env.example` параметрами (`enabled`, maxHistoryTokens, defaultTargetLanguage). → `backend-mcp/src/main/java/com/aiadvent/mcp/backend/config/GitHubRagProperties.java`, `backend-mcp/src/main/java/com/aiadvent/mcp/backend/github/rag/RepoRagSearchService.java`, `backend-mcp/src/main/resources/application-github.yaml`, `.env.example`
- [ ] Внедрить `MultiQueryExpander` + `ConcatenationDocumentJoiner` в Retrieval слое: генерировать N подзапросов, выполнять similarity search параллельно по namespace, объединять результаты с дедупликацией по `chunk_hash`, добавив конфиг `github.rag.multi-query.{enabled,maxQueries}` и метрики времени на экспансию. → `backend-mcp/src/main/java/com/aiadvent/mcp/backend/github/rag/RepoRagSearchService.java`, `backend-mcp/src/main/java/com/aiadvent/mcp/backend/github/rag/RepoRagRetrievalPipeline.java`, `backend-mcp/src/main/resources/application-github.yaml`
- [ ] Реализовать Post-Retrieval процессинг через Spring AI `DocumentPostProcessor`: обобщить текущий heuristic rerank, добавить обрезку по `ContextWindowBudget`, опциональное LLM-компрессирование snippet'ов и плагины для будущих rerank-моделей. → `backend-mcp/src/main/java/com/aiadvent/mcp/backend/github/rag/postprocessing/*`, `backend-mcp/src/main/java/com/aiadvent/mcp/backend/github/rag/HeuristicRepoRagSearchReranker.java`
- [ ] Добавить Generation слой (`ContextualQueryAugmenter`) с кастомными шаблонами (`resources/prompts/github-rag-context.st`, `github-rag-empty-context.st`), настройкой `allowEmptyContext`, инъекцией обязательных инструкций (цитирование путей, запрет галлюцинаций) и возможностью локализации вывода. → `backend-mcp/src/main/java/com/aiadvent/mcp/backend/github/rag/RepoRagSearchService.java`, `backend-mcp/src/main/resources/prompts/*`, `backend-mcp/src/main/resources/application-github.yaml`
- [ ] Протянуть новые параметры в конфигурацию `SearchCommand`: topK per sub-query, similarity threshold per language, лимиты на суммарный объём токенов, страхующие блокировки при отсутствии контекста. Обновить `RepoRagSearchService` и связанные DTO. → `backend-mcp/src/main/java/com/aiadvent/mcp/backend/github/rag/RepoRagSearchService.java`, `backend-mcp/src/main/java/com/aiadvent/mcp/backend/github/rag/RepoRagSearchService.SearchCommand`
- [ ] Переработать слой чанкинга: выделить `ChunkingStrategy` c адаптером под Spring AI `DocumentSplitter`, добавить поддержку overlap, сохранение line offsets и детерминированных `chunk_id`, выбор стратегии (line-based/byte-based/token-based) через конфиг. → `backend-mcp/src/main/java/com/aiadvent/mcp/backend/github/rag/chunking/*`, `backend-mcp/src/main/java/com/aiadvent/mcp/backend/github/rag/RepoRagIndexService.java`, `backend-mcp/src/main/resources/application-github.yaml`
- [ ] Добавить «semantic chunking» режим для кода: эвристики по языкам (поиск сигнатур функций/классов, комментариев `///`, `def`, `function`), возможность подтягивать соседние чанки в респонсе и расширить метаданные (parent symbol, span hash). Включить fallback на line-based при отсутствии AST-поддержки. → `backend-mcp/src/main/java/com/aiadvent/mcp/backend/github/rag/chunking/SemanticCodeChunker.java`, `backend-mcp/src/test/java/com/aiadvent/mcp/backend/github/rag/ChunkingStrategyTest.java`, `docs/architecture/github-rag-modular.md`

### Backend (core) и MCP инструменты
- [ ] Расширить `repo.rag_search` контракт дополнительными полями (`useCompression`, `translateTo`, `multiQuery`, `filterExpression`, `allowEmptyContext`, `maxContextTokens`) с валидацией и описанием по умолчанию; обновить DTO/ответ так, чтобы отражать использованные модули и предупреждать при пустом контексте. → `backend-mcp/src/main/java/com/aiadvent/mcp/backend/github/rag/RepoRagTools.java`, `backend/src/main/resources/db/changelog/db.changelog-master.yaml`
...
- [ ] Обновить `docs/guides/mcp-operators.md`, `docs/architecture/coding-assistant-rfc.md`, `README.md`: параметры новых фич, примеры CLI/чата, чек-листы расследования (как понять, что сработали QueryTransformers, сколько подзапросов ушло, что делать при пустом контексте). Добавить ссылку на Spring AI Modular RAG доки (`local-rag/spring-ai/docs.spring.io/...`) для справки. → `docs/guides/mcp-operators.md`, `docs/architecture/coding-assistant-rfc.md`, `README.md`

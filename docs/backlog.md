...
Цель: перестроить пост-обработку `repo.rag_search`, чтобы она понимала структуру кода и умела расширять выдачу близлежащими чанками без повторного запроса в Vector Store. Это снизит количество промахов в top‑K, улучшит покрытия class/function-level запросов и даст более стабильный контекст для генерации.

### AST-aware indexing & metadata
- [ ] Подключить Tree-sitter как единый движок разбора кода: добавить модуль `TreeSitterAnalyzer` с пулом парсеров для Java/Kotlin/TS/JS/Python/Go (первую волну), хранить скомпилированные грамматики в `backend-mcp` и загружать их через JNI/JNA. В `GitHubRagProperties` описать `github.rag.ast.{enabled,languages[],libraryPath}` и fallback на текущие эвристики при отсутствии грамматики или ошибке парсера. → `backend-mcp/src/main/java/com/aiadvent/mcp/backend/github/rag/ast/*`, `backend-mcp/build.gradle`, `backend-mcp/Dockerfile`
- [ ] Расширить `ChunkableFile`/`SemanticCodeChunker`: до начала чанкинга строим AST, определяем `symbol_kind`, `symbol_fqn`, docstring, границы тела и сохраняем в `Chunk`/`Document` метаданные `symbol_fqn`, `symbol_kind`, `symbol_signature`, `symbol_visibility`, `is_test`, `imports`. Для каждой функции собираем `calls_out[]` (список FQN вызываемых символов) и `calls_in[]` (если символ найден в других файлах). Метаданные сериализуем в JSON, чтобы ими могли пользоваться пост-процессоры и клиенты. → `backend-mcp/src/main/java/com/aiadvent/mcp/backend/github/rag/chunking/*`, `RepoRagIndexService.java`, `RepoRagDocumentEntity`
- [ ] Построить индекс вызовов: либо отдельная таблица `repo_rag_symbol_graph` (namespace, symbol_fqn, referenced_symbol_fqn, relation) либо вложенный JSON в `RepoRagDocumentEntity.metadata`. Реализовать сервис `RepoRagSymbolService`, чтобы быстро спрашивать «какие чанки содержат вызовы символа X» и «где определён символ Y». → `backend-mcp/src/main/java/com/aiadvent/mcp/backend/github/rag/persistence/*`, `backend-mcp/src/main/java/com/aiadvent/mcp/backend/github/rag/RepoRagSymbolService.java`
- [ ] Обновить pipeline пост-обработки: `NeighborChunkDocumentPostProcessor` и будущие code-aware шаги должны уметь подтягивать не только соседей по файлу, но и `calls_out`/`calls_in`. Добавить стратегию `neighborChunkStrategy=CALL_GRAPH`, которая по метаданным достаёт реализацию вызываемого метода и вставляет её в выдачу. → `backend-mcp/src/main/java/com/aiadvent/mcp/backend/github/rag/postprocessing/*`, `application-github.yaml`
- [ ] Тесты и enablement: интеграционные тесты индексации (мини-проекты на Java/TS/Python) с проверкой `symbol_fqn` и call graph, smoke-тест загрузки Tree-sitter либ в CI, документация для операторов о новых полях (`symbol_fqn`, `calls_out`, `neighborOfSpanHash`). → `backend-mcp/src/test/java/com/aiadvent/mcp/backend/github/rag/*`, `docs/guides/mcp-operators.md`, `docs/architecture/github-rag-modular.md`, `README.md`

### Backend MCP — rerank pipeline
- [ ] Добавить `CodeAwareDocumentPostProcessor`, который анализирует `parent_symbol`, `language`, `file_path`, `chunk_index`, `generatedBySubQuery` и метаданные запроса. Правила: приоритет сигнатур/классов, штрафы для `generated/`, `testdata`, `node_modules`, диверсификация по `file_path`/`parent_symbol`, бонус за совпадение языка запроса. Процессор встраивается в `HeuristicRepoRagSearchReranker.buildProcessors(...)` сразу после базовой эвристики и до `ContextWindowBudget`; конфиг (`github.rag.rerank.codeAware.*`) описывает веса, лимиты per-file, allow-list/deny-list директорий. → `backend-mcp/src/main/java/com/aiadvent/mcp/backend/github/rag/postprocessing/CodeAwareDocumentPostProcessor.java`, `backend-mcp/src/main/java/com/aiadvent/mcp/backend/github/rag/HeuristicRepoRagSearchReranker.java`, `backend-mcp/src/main/resources/application-github.yaml`
- [ ] Прокинуть новые параметры в `RepoRagPostProcessingRequest`/`SearchCommand`: `codeAwareEnabled` (bool, default true) и `codeAwareHeadMultiplier` (сколько элементов анализируем относительно `rerankTopN`). Обновить DTO инструментов (`RepoRagSearchInput`, `RepoRagGlobalSearchInput`) и валидацию (`RepoRagSearchService.validate*`). Документация MCP должна отражать появившиеся параметры и дефолтные лимиты. → `backend-mcp/src/main/java/com/aiadvent/mcp/backend/github/rag/RepoRagPostProcessingRequest.java`, `RepoRagTools.java`, `RepoRagSearchService.java`, `docs/guides/mcp-operators.md`, `docs/architecture/github-rag-modular.md`

### Neighbor chunk expansion
- [ ] Реализовать `NeighborChunkDocumentPostProcessor`: при наличии `neighborChunkRadius>0` собирает `(namespace, file_path, chunk_index±radius)` для текущих документов, батчом вытягивает недостающие чанки из `repo_rag_vector_store` (через новый метод `RepoRagDocumentRepository.findByNamespaceAndFilePathAndChunkIndexIn`) и вставляет их рядом с оригиналом. Нужно помечать добавленные элементы (`metadata.neighborOfSpanHash`, `appliedModules+=post.neighbor-expand`), не дублировать существующие `chunk_hash` и соблюдать общий лимит `neighborChunkLimit` (штатно 6). → `backend-mcp/src/main/java/com/aiadvent/mcp/backend/github/rag/postprocessing/NeighborChunkDocumentPostProcessor.java`, `backend-mcp/src/main/java/com/aiadvent/mcp/backend/github/rag/persistence/RepoRagDocumentRepository.java`, `RepoRagSearchReranker.java`
- [ ] Расширить `RepoRagPostProcessingRequest` и конфигурацию: `neighborChunkRadius`, `neighborChunkLimit`, флаг `neighborChunkStrategy` (enum: `OFF`, `PARENT_SYMBOL`, `LINEAR`). Значения берём из `github.rag.post-processing.neighbor.*`, а для инструментов вводим параметры `neighborRadius`/`neighborLimit` (валидация ≤5 и ≤12 соответственно). Сервис `RepoRagSearchService` должен передавать настройки в реранкер, а `repo.rag_search`/`repo.rag_search_global` — возвращать новый applied-module, если соседи были добавлены. → `backend-mcp/src/main/resources/application-github.yaml`, `RepoRagTools.java`, `RepoRagSearchService.java`, `RepoRagPostProcessingRequest.java`

### Persistence, тесты и документация
- [ ] Добавить конвертера `RepoRagDocumentMapper`, который превращает `RepoRagDocumentEntity` из базы в `Document` Spring AI (учитываем JSON-метадату и score при наличии). Использовать его в соседнем пост-процессоре. Нужны интеграционные тесты на «добавлен сосед по parent_symbol», «отключён neighbor expansion», «лимит превысить нельзя». → `backend-mcp/src/main/java/com/aiadvent/mcp/backend/github/rag/persistence/RepoRagDocumentMapper.java`, `backend-mcp/src/test/java/com/aiadvent/mcp/backend/github/rag/postprocessing/NeighborChunkDocumentPostProcessorTest.java`
- [ ] Обновить `docs/guides/mcp-operators.md`, `docs/architecture/github-rag-modular.md`, `README.md`: описать новый code-aware rerank, параметры `neighborRadius/neighborLimit`, влияние на `appliedModules`, best practices по дебагу (как понять, откуда пришли соседи), SLA и лимиты. Добавить примеры ответов с пометкой `neighborOfSpanHash`. → `docs/guides/mcp-operators.md`, `docs/architecture/github-rag-modular.md`, `README.md`

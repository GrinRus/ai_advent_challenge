...
- Runtime: Java binding `ch.usi.si.seart:java-tree-sitter` с загрузкой собранных грамматик из `treesitter/<os>/<arch>/libtree-sitter-*.{so,dylib}`. `TreeSitterLibraryLoader` копирует библиотеки во временный каталог и отдаёт `Path` для `Language.fromPath`, с graceful fallback на эвристику при отсутствии артефактов.
...
- [ ] Подключить нативный Tree-sitter через JavaCPP: добавить собственный JavaCPP-presets для C API (`TSParser/TSLanguage/TSNode`), грузить грамматики из `treesitter/*`, читать `github.rag.ast.native-enabled`, оставляя fallback. Поддерживаем Java/Kotlin/TS/JS/Python/Go.
- [ ] Переписать `TreeSitterParser` на реальный обход AST: package/imports, классы/функции, сигнатуры аргументов, модификаторы, docstring; FQN `package.Class#method(args)`; edges `CALLS/IMPLEMENTS/READS_FIELD/USES_TYPE`.

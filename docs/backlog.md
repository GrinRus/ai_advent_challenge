...
### Принятые решения
- **Финальная доставка ответов**: в Telegram показываем только завершённый ответ; никаких частичных токенов или live-редакций. Backend завершает генерацию при остановке пользователем, но бот буферизует текст до финала.
  1. `LLM stream → Telegram` (буфер на стороне бота): проксируем `POST /api/llm/chat/stream`, копим события `token`, публикуем лишь `complete`, сохраняя метаданные (`usage`, `cost`, `tools`).
  2. `LLM sync → Telegram`: для чата используем `POST /api/llm/chat/sync` (сервис `SyncChatService`), отдавая пользователю только готовый ответ и structured payload.
  3. `Stream facade`: отдельный backend-адаптер сам дожидается `complete` и возвращает итоговый JSON; бот общается с фасадом без SSE.
...
- [ ] Выбрать стратегию доставки финального ответа (буферизация SSE, sync-запрос или фасад) и закрепить реализацию в адаптере Telegram.

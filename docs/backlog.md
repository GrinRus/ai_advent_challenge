...
### Принятые решения
- **Стриминг**: в Telegram показываем только финальные ответы; никаких частичных токенов или live-редакций, backend достаточно досрочно завершить доставку текущей генерации.
...
- **Сессии**: используем общий UUID `chat_session`, чтобы диалоги Telegram и веб-UI могли разделять историю.
- **STT**: для голосовых сообщений предусматриваем omni-модели OpenAI (GPT-4o / GPT-4o mini omni) и z.ai (GLM-4.6 omni) как основные варианты, с возможностью добавить fallback (например, OpenAI Whisper или локальный движок), оборачивая их в единый интерфейс.

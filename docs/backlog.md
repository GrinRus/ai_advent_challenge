...
- [ ] Дополнить `docs/guides/mcp-operators.md` runbook'ом: как отслеживать прогресс (poll `repo.rag_index_status`), как интерпретировать поля ответа, как использовать `repo.rag_search` для отбора релевантного контекста с heuristic rerank и как реагировать на ошибки индексации. → `docs/guides/mcp-operators.md`
- [ ] Расширить `docs/architecture/coding-assistant-rfc.md` новым e2e-кейсом (`github.repository_fetch` → ожидание индекса → `repo.rag_search` → `coding.generate_patch`) и описать, как отсортированные чанки (без отдельной модели) влияют на подсказки агента. → `docs/architecture/coding-assistant-rfc.md`

...
### Backend
- [ ] Persist structured sync ответы в `chat_message`: добавить JSONB-колонку для `structured_payload`, сохранять её из `StructuredSyncService`, расширить ORM/репозитории и обеспечить миграцию.
- [ ] Расширить стратегию ретраев: добавить в `ChatProvidersProperties` секцию `retry` (попытки, начальный backoff, множитель, retryable status codes) и конфигурировать `RetryTemplate` для каждого провайдера на основе этих параметров.
- [ ] Доработать `StructuredSyncService` для использования новых настроек и логирования количества попыток/причины остановки.
- [ ] Обновить интеграционные/модульные тесты: проверка сохранения structured payload, применение кастомных ретраев и обработки 422/429/5xx.

### Frontend
- [ ] Отобразить сохранённые structured ответы в истории сессии: при загрузке чата показывать карточки из `structured_payload` рядом с текстовыми сообщениями.

### Документация
- [ ] Обновить `docs/infra.md` и `docs/processes.md`: схема новой колонки, пример настроек `retry`, требования к логам и тестам.

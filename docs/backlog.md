...
- [ ] Расширить `GitHubRepositoryService`/`GitHubTools` и зарегистрировать самостоятельный MCP-инструмент `github_repository_fetch` в том же GitHub MCP-сервере, что и текущие интеграции: вход — repo/ref/checkout strategy, выход — ссылка на подготовленный архив/каталог в локальном storage. Зафиксировать DTO запроса/ответа и описание в `@Tool` (назначение, ограничения, примеры).
- [ ] Обновить `GitHubRepositoryService` методами `fetchRepositoryTo(Path target, GitFetchOptions)` с поддержкой archive download (ZIP/TAR), fallback на git clone при необходимости сабмодулей/LFS, реиспользуя PAT из конфигурации.
- [ ] Добавить ограничители для GitHub-интеграции (максимальный размер архива, таймаут загрузки, маскирование токена) и заголовки источника (`repo/ref/commitSha`) с сохранением метаданных в промежуточном хранилище (`TempWorkspaceService`).
...
- [ ] Возвращать из инструмента расширенный ответ: `workspaceId`, абсолютный путь, размер выгрузки, время скачивания, commit SHA, список ключевых файлов; фиксировать события/метрики (`download_time_ms`, `download_size_bytes`, ошибки таймаута/лимита).
...
### Agents & Flow
- [ ] Создать специализированного агента `repo-gradle-tester`: системная подсказка описывает цепочку (`github_repository_fetch` → `workspace_directory_inspector` → `docker_gradle_runner`), список требуемых аргументов и валидацию входа (repo URL, ref, Gradle tasks).
- [ ] Добавить вспомогательного агента/режим `workspace-navigator`, который помогает пользователю исследовать workspace и выбирать `projectPath`, используя инспектор и свободный ввод.
- [ ] Сконструировать новый flow `github-gradle-test-flow` со свободным вводом: сбор параметров от пользователя, последовательный вызов инструментов, обработка промежуточных результатов (список проектов) и отображение итогового отчёта.
- [ ] Обновить UI/DTO Flow каталога: добавить описание нового flow, форму для ввода repo/ref/tasks/optional projectPath, подсказки по выбору инструмента.
- [ ] Написать e2e сценарий flow: пользователь вводит repo/ref, агент вызывает инструменты в нужном порядке, рендерятся промежуточные статусы и финальный отчёт.
- [ ] Документировать сценарии использования агентов и flow в `docs/guides/mcp-operators.md`: пример диалога/flow запуска, ограничения, рекомендации по выбору задач.

### Observability & QA
- [ ] Логировать метрики по каждому инструменту отдельно (GitHub fetch, Docker run) с корреляцией через `requestId`: время, объём данных, коды возврата, ошибки.
...
- [ ] Обновить Flow e2e сценарии: проверка отображения промежуточных статусов (`fetching`, `running_tests`), координация последовательных вызовов инструментов на стороне агента и graceful degradation при сбоях.

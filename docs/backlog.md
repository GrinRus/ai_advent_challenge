...
- [ ] Подключить Tree-sitter как единый движок разбора кода: добавить модуль `TreeSitterAnalyzer`, который через Gradle task `treeSitterBuild` собирает pinned грамматики (Java/Kotlin/TS/JS/Python/Go) и раскладывает `.so/.dylib/.dll` по `os/arch` в `src/main/resources/treesitter/**`. Все грамматики тянем только как git submodules на официальные `tree-sitter-*` репозитории (фиксированный commit + запись в `gradle/libs.versions.toml`), так что сборка повторяема и оффлайн. Во время `bootJar` копируем либы рядом с jar, а в рантайме загружаем через `System.load`, читая конфиг `github.rag.ast.{enabled,languages[],libraryPath}`. В Docker образ кладём только linux-вариант; при ошибке загрузки автоматически фолбэкаемся на текущую эвристику без алертов. → `backend-mcp/build.gradle`, `backend-mcp/Dockerfile`, `backend-mcp/src/main/java/com/aiadvent/mcp/backend/github/rag/ast/*`, `backend-mcp/src/main/resources/application-github.yaml`
  - [ ] Завести git submodules `backend-mcp/treesitter/tree-sitter-{java,kotlin,typescript,javascript,python,go}` и зафиксировать commit hash + checksum в `gradle/libs.versions.toml`; `README.md`/`CONTRIBUTING.md` пополнить инструкциями `git submodule update --init`.
  - [ ] Добавить Gradle-задачу `treeSitterBuild` (и `treeSitterVerify`), которая собирает `.so/.dylib/.dll` в `build/treesitter/<os>/<arch>`, проверяет checksum, а при `processResources` синхронизирует артефакты в `src/main/resources/treesitter/**`.
  - [ ] Реализовать `TreeSitterAnalyzer` (SPI + JNI loader): конфиг `github.rag.ast.*`, lazy `System.load`, health-проба, счётчик consecutive failures + fallback на эвристический `ParentSymbolResolver`.
...
- [ ] Расширить `ChunkableFile`/`SemanticCodeChunker`: при построении `ChunkableFile` готовим `AstFileContext` (ленивый Tree-sitter AST + индекс символов). Чанкер получает готовый AST, обогащает `Chunk` метаданными `symbol_fqn`, `symbol_kind`, `symbol_signature`, `symbol_visibility`, `docstring`, `is_test`, `imports[]`, `calls_out[]`, `calls_in[]`. В `RepoRagIndexService` сериализуем поля в `metadata` JSON и проставляем `ast_available`. При падении парсинга — логируем debug и используем текущий `ParentSymbolResolver`. → `backend-mcp/src/main/java/com/aiadvent/mcp/backend/github/rag/chunking/*`, `RepoRagIndexService.java`, `backend-mcp/src/main/java/com/aiadvent/mcp/backend/github/rag/persistence/RepoRagDocumentEntity.java`
  - [ ] Расширить `Chunk` record и `RepoRagDocumentEntity.metadata` schema, добавить mapper из AST модели → `ChunkMetadata`; `Chunk` хранит ссылку на `AstSymbolMetadata` (FQN, сигнатура, docstring, visibility, test-flag, counters).
  - [ ] `ChunkableFile` и `SemanticCodeChunker` внедряют `TreeSitterAnalyzer`: один проход по AST формирует `AstFileContext`, который кэшируется внутри `ChunkableFile` и раздаёт символы/границы/импорты всем стратегиям.
...
- [ ] Построить отдельный индекс вызовов: таблица `repo_rag_symbol_graph` (`id`, `namespace`, `file_path`, `symbol_fqn`, `symbol_kind`, `chunk_hash`, `referenced_symbol_fqn`, `relation`, `created_at`) + сервис `RepoRagSymbolService`, который выдаёт «где объявлен символ» и «какие чанки его вызывают». Добавить индексы по `(namespace,symbol_fqn)` и `(namespace,referenced_symbol_fqn)`. → `backend-mcp/src/main/java/com/aiadvent/mcp/backend/github/rag/persistence/*`, `backend-mcp/src/main/java/com/aiadvent/mcp/backend/github/rag/RepoRagSymbolService.java`, Liquibase
  ...
  - [ ] Добавить `SymbolGraphWriter`, который вызывается сразу после `vectorStore.replaceFile`/`deleteFile` в `RepoRagIndexService` и синхронизирует граф по `namespace+file_path`: diff старых/новых рёбер, батчевые `saveAll`/`deleteByNamespaceAndFilePath`.
  - [ ] `SymbolGraphWriter` читает `ChunkMetadata.calls_out` и строит рёбра `symbol_fqn -> referenced_symbol_fqn`, а при переиндексации файла удаляет прежние записи.
  - [ ] Расширить `RepoRagSymbolService`: `findSymbolDefinition`, `findOutgoingEdges`, батч-кэширование (Caffeine) и троттлинг; включение стратегии `CALL_GRAPH` в `NeighborChunkDocumentPostProcessor` только если namespace помечен `astReady`.

...
- [ ] Подключить нативный Tree-sitter через JavaCPP: добавить зависимость/пресет (TSParser/TSLanguage), загрузку нативных либ из `treesitter/*`, флаг `github.rag.ast.native-enabled` и fallback на эвристику. Поддерживаем языки Java/Kotlin/TS/JS/Python/Go.
- [ ] Реализовать обход AST в `TreeSitterParser`: извлечение package/imports, классов/функций, сигнатур аргументов, модификаторов, docstring; построение FQN `package.Class#method(args)` и edges `CALLS/IMPLEMENTS/READS_FIELD/USES_TYPE`.
- [ ] Расширить fixtures `backend-mcp/src/test/resources/mini-repos/{java,kotlin,typescript,javascript,python,go}` (наследование, перегрузки, interfaces/implements) и обновить тесты (`RepoRagIndexServiceMultiLanguageTest`, AST unit) на проверки FQN, docstring, edges, ошибок парсинга.

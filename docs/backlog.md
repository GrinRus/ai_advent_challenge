...
- [x] Зафиксировать архитектуру в `docs/architecture/mcp-gradle.md`: поток `fetch → package → docker-run → отчёт`, контракты между инструментами, последовательность статусов, координацию на стороне LLM/flow.

### Workspace Inspector Tool
- [x] Обновить документацию и каталоги MCP: описать инструмент, привести примеры запросов (поиск Gradle, поиск произвольных файлов), уточнить порядок использования (fetch → inspect → docker run) для много-репозиторных сценариев.

### Docker Runner Tool
- [x] Написать интеграционный тест `DockerMcpApplicationTests`: поднять временный workspace с Gradle-проектом, выполнить `docker_gradle_runner`, убедиться в корректной передаче логов, статуса и таймаутов.

### Agents & Flow
- [x] Написать e2e сценарий flow: пользователь вводит repo/ref, flow вызывает инструменты в нужном порядке, отображаются промежуточные статусы и финальный отчёт.
- [x] Документировать использование агентов и flow в `docs/guides/mcp-operators.md`: примеры диалогов, порядок вызовов, ограничения и рекомендации.

### Observability & QA
- [x] Логировать метрики по каждому инструменту отдельно (GitHub fetch, Workspace inspector, Docker run) с корреляцией через `requestId`: время, объём данных, коды возврата, ошибки.
- [x] Написать интеграционные тесты: успешный прогон sample Gradle-проекта, ошибка сборки в Docker, таймаут контейнера, несуществующий репозиторий, превышение лимита размера архива.
- [x] Обновить Flow e2e сценарии: проверка отображения промежуточных статусов (`fetching`, `inspecting_workspace`, `running_tests`), координация последовательных вызовов инструментов на стороне flow и graceful degradation при сбоях.

### Документация & Enablement
- [x] Обновить `docs/guides/mcp-operators.md` и `docs/infra.md`: описать новые инструменты, их независимость, параметры Docker-окружения и требования к PAT.
- [x] Дополнить документацию детальными карточками каждого инструмента (назначение, поля входа/выхода, типичные сценарии, ограничения) и примером JSON-вызовов, чтобы LLM/операторы могли корректно их использовать.
...
### Backend (backend-mcp)
- [x] Ввести интерфейс `PatchGenerator` и реализацию `ClaudeCliPatchGenerator`, которая собирает JSON-команду (`instructions`, `targetPaths`, `forbiddenPaths`, `contextSnippets`) и запускает `claude -p <payload> --output-format json` (модель GLM-4.6) внутри workspace. Проверить, что возвращаемый JSON содержит `summary`, `diff` (unified), `files[]`, `risks[]`, `usage.promptTokens/completionTokens`; навесить валидации лимитов (`coding.max-*`). → `backend-mcp/src/main/java/com/aiadvent/mcp/backend/coding/*`
- [x] Добавить `ClaudeCliService`: поиск бинаря по `CLAUDE_CODE_BIN` или `$PATH`, валидация (`claude --version`), прокидка `ZHIPU_API_KEY` в `ANTHROPIC_API_KEY/ANTHROPIC_AUTH_TOKEN`, конфигурация таймаутов (`CLAUDE_CODE_TIMEOUT`, `CLAUDE_CODE_API_TIMEOUT`), настройка дефолтных моделей (`ANTHROPIC_DEFAULT_*`) и запуск через `ProcessBuilder` с отдельными потоками stdout/stderr и очисткой ANSI-последовательностей перед парсингом. Вернуть типизированный ответ/ошибку (timeout, non-zero exit, stderr warning). → `backend-mcp/src/main/java/com/aiadvent/mcp/backend/coding`
- [x] Зафиксировать работу с GLM (z.ai): вынести в свойства `coding.claude.base-url`, `coding.claude.model`, `coding.claude.api-key`, по умолчанию использовать `https://api.z.ai/api/anthropic` и модели `GLM-4.6/GLM-4.5/GLM-4.5-Air`, маппируя «balanced/fast/advanced» профили coding flow на соответствующие названия GLM. → `backend-mcp/src/main/java/com/aiadvent/mcp/backend/coding/CodingAssistantProperties.java`, `ClaudeCliService`
- [x] Использовать `WorkspaceFileService` для подготовки контекста CLI: читать snippets в пределах `coding.max-context-bytes`, складывать промежуточный `claude-context.json`/`claude-prompt.md` во временную директорию workspace, очищать артефакты после завершения и логировать структуру промпта без содержимого. → `backend-mcp/src/main/java/com/aiadvent/mcp/backend/workspace/WorkspaceFileService.java`
- [x] Перенастроить `CodingAssistantService` на новую реализацию генератора: внедрить `PatchGenerator`, логировать размеры prompt/diff, фильтровать бинарные блоки, обновлять `PatchRegistry` и флаг `requiresManualReview` при ошибках CLI или предупреждениях. Предусмотреть feature-flag для fallback на patch-plan. → `backend-mcp/src/main/java/com/aiadvent/mcp/backend/coding/CodingAssistantService.java`, `CodingAssistantProperties.java`, `PatchRegistry.java`
- [x] Нормализовать usage/аннотации из CLI-ответа: маппинг `files[]` → `PatchAnnotations.files`, `risks[]` → `PatchAnnotations.risks`, отдельное поле `warnings` для dry-run ограничений; usage сохранять в `PatchUsage` и экспонировать в ответах MCP-инструментов/метриках. → `backend-mcp/src/main/java/com/aiadvent/mcp/backend/coding/*`
- [ ] Покрыть тестами: unit на `ClaudeCliService` (успех, stderr, timeout, смена модели), unit на `ClaudeCliPatchGenerator` (некорректный JSON, превышение лимитов, бинарные diff'ы), интеграционный сценарий generate→review→apply с заглушкой CLI (скрипт, возвращающий детерминированный JSON). → `backend-mcp/src/test/java/com/aiadvent/mcp/backend/coding/*`

### Инфраструктура и конфигурация
- [x] Добавить переменные `CLAUDE_CODE_BIN`, `CLAUDE_CODE_TIMEOUT`, `CODING_CLAUDE_ENABLED`, `CODING_CLAUDE_BASE_URL`, `CODING_CLAUDE_MODEL`, `ZHIPU_API_KEY` в `.env.example`, `application-coding.yaml` (по умолчанию `https://api.z.ai/api/anthropic`). → `.env.example`, `backend-mcp/src/main/resources/application-coding.yaml`
- [x] Обновить `docker-compose.yml` и образ `coding-mcp`: установить Node.js 18+/npm, `npm install -g @anthropic-ai/claude-code`, смонтировать кеш `~/.npm`, прокинуть `ZHIPU_API_KEY`, добавить healthcheck `claude --version` и опцию для внешнего CLI volume. → `docker-compose.yml`, `backend-mcp/Dockerfile`
- [x] Создать раздел в `docs/infra.md` (dev/prod): требования к Node/npm, установка CLI, управление ключами `ZHIPU_API_KEY`, проверка `claude --version`, настройка GLM endpoint (`https://api.z.ai/api/anthropic`, модели GLM-4.5/GLM-4.6), распространение обновлений, очистка кешей. → `docs/infra.md`

### Наблюдаемость и безопасность
- [x] Добавить структурированное логирование генерации патча: фиксировать `promptBytes`, `contextBytes`, `diffBytes`, статус CLI (ok/timeout/error), маскировать stderr и usage перед выводом. → `backend-mcp/src/main/java/com/aiadvent/mcp/backend/coding/CodingAssistantService.java`
...
- [x] Подключить Tree-sitter как единый движок разбора кода: добавить модуль `TreeSitterAnalyzer`, который через Gradle task `treeSitterBuild` собирает pinned грамматики (Java/Kotlin/TS/JS/Python/Go) и раскладывает `.so/.dylib/.dll` по `os/arch` в `src/main/resources/treesitter/**`. Все грамматики тянем только как git submodules на официальные `tree-sitter-*` репозитории (фиксированный commit + запись в `gradle/libs.versions.toml`), так что сборка повторяема и оффлайн. Во время `bootJar` копируем либы рядом с jar, а в рантайме загружаем через `System.load`, читая конфиг `github.rag.ast.{enabled,languages[],libraryPath}`. В Docker образ кладём только linux-вариант; при ошибке загрузки автоматически фолбэкаемся на текущую эвристику без алертов. → `backend-mcp/build.gradle`, `backend-mcp/Dockerfile`, `backend-mcp/src/main/java/com/aiadvent/mcp/backend/github/rag/ast/*`, `backend-mcp/src/main/resources/application-github.yaml`
  - [x] Завести git submodules `backend-mcp/treesitter/tree-sitter-{java,kotlin,typescript,javascript,python,go}` и зафиксировать commit hash + checksum в `gradle/libs.versions.toml`; `README.md`/`CONTRIBUTING.md` пополнить инструкциями `git submodule update --init`.
  - [x] Добавить Gradle-задачу `treeSitterBuild` (и `treeSitterVerify`), которая собирает `.so/.dylib/.dll` в `build/treesitter/<os>/<arch>`, проверяет checksum, а при `processResources` синхронизирует артефакты в `src/main/resources/treesitter/**`.
  - [x] Реализовать `TreeSitterAnalyzer` (SPI + JNI loader): конфиг `github.rag.ast.*`, lazy `System.load`, health-проба, счётчик consecutive failures + fallback на эвристический `ParentSymbolResolver`.
  - [x] Обновить `bootJar`/`Dockerfile`, чтобы jar получал только требуемые библиотеки, а linux-вариант копировался в рантайм-образ; в `README.md` описать необходимость пересборки грамматик при смене ОС и шаг `./gradlew treeSitterBuild` перед `bootJar`.
  - [x] Добавить smoke-check (Gradle `treeSitterVerify` + CI job), что библиотеки реально загружаются на Linux и macOS и печатают версии грамматик.
- [x] Расширить `ChunkableFile`/`SemanticCodeChunker`: при построении `ChunkableFile` готовим `AstFileContext` (ленивый Tree-sitter AST + индекс символов). Чанкер получает готовый AST, обогащает `Chunk` метаданными `symbol_fqn`, `symbol_kind`, `symbol_signature`, `symbol_visibility`, `docstring`, `is_test`, `imports[]`, `calls_out[]`, `calls_in[]`. В `RepoRagIndexService` сериализуем поля в `metadata` JSON и проставляем `ast_available`. При падении парсинга — логируем debug и используем текущий `ParentSymbolResolver`. → `backend-mcp/src/main/java/com/aiadvent/mcp/backend/github/rag/chunking/*`, `RepoRagIndexService.java`, `backend-mcp/src/main/java/com/aiadvent/mcp/backend/github/rag/persistence/RepoRagDocumentEntity.java`
  - [x] Расширить `Chunk` record и `RepoRagDocumentEntity.metadata` schema, добавить mapper из AST модели → `ChunkMetadata`; `Chunk` хранит ссылку на `AstSymbolMetadata` (FQN, сигнатура, docstring, visibility, test-flag, counters).
  - [x] `ChunkableFile` и `SemanticCodeChunker` внедряют `TreeSitterAnalyzer`: один проход по AST формирует `AstFileContext`, который кэшируется внутри `ChunkableFile` и раздаёт символы/границы/импорты всем стратегиям.
  - [x] Интегрировать в `RepoRagIndexService`: проставлять `ast_available`, сериализовать массивы `imports/calls_*`, сохранять `symbol_*`, `is_test`, `docstring`, `ast_version`; добавить `repo_rag_metadata_schema` (таблица с версиями JSON Schema) и миграцию `metadata.schema_version`.
    - [x] Добавить changelog `repo_rag_metadata_schema` и записывать `metadata_schema_version`/`ast_version` в документы.
  - [x] Добавить graceful fallback: если язык не поддержан или AST не загрузился, логируем `debug` + счётчик и используем прежний `ParentSymbolResolver`.
- [x] Построить отдельный индекс вызовов: таблица `repo_rag_symbol_graph` (`id`, `namespace`, `file_path`, `symbol_fqn`, `symbol_kind`, `chunk_hash`, `referenced_symbol_fqn`, `relation`, `created_at`) + сервис `RepoRagSymbolService`, который выдаёт «где объявлен символ» и «какие чанки его вызывают». Добавить индексы по `(namespace,symbol_fqn)` и `(namespace,referenced_symbol_fqn)`. → `backend-mcp/src/main/java/com/aiadvent/mcp/backend/github/rag/persistence/*`, `backend-mcp/src/main/java/com/aiadvent/mcp/backend/github/rag/RepoRagSymbolService.java`, Liquibase
  - [x] Дополнить Liquibase changelog: новая таблица + on-delete каскад по namespace, индексы и rollback.
  - [x] Добавить `SymbolGraphWriter`, который вызывается сразу после `vectorStore.replaceFile`/`deleteFile` в `RepoRagIndexService` и синхронизирует граф по `namespace+file_path`: diff старых/новых рёбер, батчевые `saveAll`/`deleteByNamespaceAndFilePath`.
  - [x] `SymbolGraphWriter` читает `ChunkMetadata.calls_out` и строит рёбра `symbol_fqn -> referenced_symbol_fqn`, а при переиндексации файла удаляет прежние записи.
  - [x] Расширить `RepoRagSymbolService`: `findSymbolDefinition`, `findOutgoingEdges`, батч-кэширование (Caffeine) и троттлинг; включение стратегии `CALL_GRAPH` в `NeighborChunkDocumentPostProcessor` только если namespace помечен `astReady`.
  - [x] Добавить CLI/observability hook (log + metric), который показывает сколько call-edges создано и сколько запросов в минуту обслуживает сервис.
- [x] Зафиксировать, что новая AST/metadata логика применяется только к свежим индексам: никаких фоновых backfill-job. Обновить документацию, чтобы операторы понимали, что уже проиндексированные namespace остаются в старом формате до следующего fetch/index. → `docs/guides/mcp-operators.md`, `docs/architecture/github-rag-modular.md`
...
### Backend (backend-mcp)
- [x] Скорректировать профили `conservative`/`aggressive`: задать `minScoreFallback`, список `overviewBoostKeywords`, обязательный override multi-query, обновить `application-github.yaml` и unit-тесты `RagParameterGuard`, чтобы высокоуровневые вопросы не заканчивались `CONTEXT_NOT_FOUND`. → `backend-mcp/src/main/java/com/aiadvent/mcp/backend/config/GitHubRagProperties.java`, `backend-mcp/src/main/java/com/aiadvent/mcp/backend/github/rag/RagParameterGuard.java`, `backend-mcp/src/test/java/com/aiadvent/mcp/backend/github/rag/RagParameterGuardTest.java`, `backend-mcp/src/main/resources/application-github.yaml`

### Документация
- [x] Дополнить `docs/architecture/github-rag-indexing.md` и `docs/guides/mcp-operators.md`: расписать `repo.rag_search_simple`, `repo.rag_search_simple_global`, автоматические fallback-сценарии, семантику `warnings`/`appliedModules` и рекомендации по выбору профиля. → `docs/architecture/github-rag-indexing.md`, `docs/guides/mcp-operators.md`

## Wave 39 — GitHub Analysis Flow production
Цель: довести GitHub Analysis Flow до продового качества: надёжный backend-пайплайн поверх GitHub MCP, единый canonical payload и полноценный UX (чат + панель) с операционными метриками и документацией.

### Backend & Flow Orchestration
- [ ] Запускать `github-analysis-flow` через flow-оркестратор: шаги `fetch_metadata → fetch_tree → analysis → recommendations` используют общий контекст резолвера, вызывают GitHub MCP (tree/file/diff), собирают статические проверки + LLM summary/риски и сохраняют structured payload для постобработки.
- [ ] Доработать `McpToolBindingService`/`GitHubResolverService`: корректный парсинг ссылок на репозиторий/ветку/PR, заполнение `repository/ref/pullRequest`, генерация `requestId`, сбор unified diff (`diff --git`/`@@`) с лимитами по размеру/количеству файлов и graceful ошибками.
- [ ] Добавить сценарий анализа pull request: сбор diff/metadata через GitHub MCP, подготовка промптов, генерация рекомендаций LLM и вычисление сигналов (компоненты, TODO, риски), единый fallback для branch vs PR.
- [ ] Обновить `AgentOrchestratorService`/`FlowInteractionService`: потоковые статусы (включая ретраи и деградацию), повторное использование промежуточных данных при рестарте и оповещения UI без перехода в FlowSessions.
- [ ] Обновить сохранение результатов анализа: структурированный payload (summary, risks, рекомендации, метрики, артефакты, ссылки), идемпотентность по URL/PR, история запусков и возможность повторного просмотра/перезапуска.
- [ ] Расширить backend DTO и OpenAPI-схемы (`analysisStatus`, `retries`, `recommendationBlocks`, `actions`, `stepProgress`), обеспечив обратную совместимость клиентов.

### GitHub MCP Coverage & Payload
- [ ] Описать canonical payload анализа (diff summary, список файлов, агрегированные метрики, сигналы, локации), зафиксировать схему в shared context/DTO и документации.
- [ ] Обновить `AgentInvocationService`/flow сценарии и каталоги: единый pipeline `github-analysis-flow`, типизированный ввод (repo URL, target type `branch|pull_request`, optional commit/ref, глубина анализа), требования к подготовке данных.
- [ ] Логировать детализированные действия GitHub MCP (вызовы методов, approve/request changes, rate limit) с привязкой к `requestId` без отдельного аудита.

### Frontend & UX
- [ ] Обновить панель MCP: отдельный блок GitHub Analysis с типизированными полями (URL, ref/PR, режим анализа), подсказками по формату, валидацией и кнопкой запуска `github-analysis-flow`.
- [ ] В чате отобразить прогресс: шаги, статусы, ошибки/ретраи, возможность перезапуска и отображение длительности этапов без переключения на FlowSessions.
- [ ] Реализовать карточку результата: summary, риски, рекомендации, список файлов с подсветкой изменений, ссылки на PR/комментарии, действия approve/request changes и статус review.
- [ ] Расширить фронтенд API-клиент/DTO под новые поля payload (`summary`, `risks`, `recommendations`, `files`, `metrics`, `links`, `reviewStatus`, `stepProgress`) и типы статусов/временных метрик.
- [ ] Добавить UI/e2e тесты: запуск анализа из чата и панели, отображение прогресса, публикация комментария и approve/request changes через GitHub MCP.

### Observability, Security & QA
- [ ] Логировать метрики по этапам анализа (fetch tree, diff build, LLM analysis, recommendations), коды возврата GitHub MCP, количество файлов, время обработки и ретраи; дублировать ключевые показатели в мониторинг.
- [ ] Покрыть GitHub MCP unit/integration тестами: handshake, tree/file чтение, pull request инструменты (list/get/diff/comments/checks), действия (create comment/review/submit), ручная сборка unified diff.
- [ ] Протестировать управление PAT: чтение из секретов, обновление без рестарта, обработка просроченных токенов и диагностические логи.
- [ ] Написать backend интеграционные тесты: успешный анализ PR, пустой/слишком большой diff, невалидная ссылка, повторный запуск с кешированием, деградация MCP.
- [ ] Обновить Flow e2e сценарии: прогон через чат и панель, проверка статусов, approve/request changes, graceful handling ошибок и идемпотентность по URL/PR.
...
- [x] Спроектировать схему `user_profile` (uuid, namespace, reference, display_name, locale, habits, anti_patterns, work_hours, metadata JSONB, timestamps) и `user_identity` (profile_id, provider, external_id, attributes JSONB, scopes) с внешними ключами и ограничениями уникальности `(provider, external_id)`. Профиль должен быть единым для пользователя (веб UI, Telegram, CLI и т.д.), а каналы лишь выбирают соответствующий `namespace/reference`. Добавить миграцию Liquibase + индексы по namespace/reference и provider.  
  **Лучший вариант:** Сохранить `namespace/reference` прямо в `user_profile` ради обратной совместимости и простых миграций, при этом `profile_lookup` служит вторичным источником правды (idempotent insert при создании/обновлении профиля).
- [x] Ввести canonical `profile_handle`: `user_profile` хранит только `profile_id` + пользовательские поля, а отдельная таблица `profile_lookup(namespace, reference, profile_id)` обслуживает сопоставление для любых каналов. Добавить уникальные индексы по `(namespace, reference)` и `profile_id`, описать стратегии миграции для будущих namespace (Slack, mobile) и гарантировать, что lookup — единственный источник правды.  
  **Лучший вариант:** Реализовать `ProfileHandleService`, который атомарно выполняет `INSERT ... ON CONFLICT DO NOTHING` и возвращает уже привязанный `profile_id`, тем самым обеспечивая идемпотентные привязки каналов.
- [x] Рассмотреть таблицу `user_profile_channel` или расширенный `metadata`, чтобы хранить отдельные настройки для Telegram/web (например, preferred reply length, включён ли синтез речи). Задокументировать, как добавлять новые поля, не ломая API и не создавая расхождений с основной записью.  
  **Лучший вариант:** Создать нормализованную таблицу `user_profile_channel` (`profile_id`, `channel`, `settings JSONB`) и маппить её через `@ElementCollection`, чтобы кешировать канал-специфичные настройки вместе с профилем.
- [x] Определить набор обязательных полей (locale, timezone, communication_mode) и валидаторы (макс. длина текстов, допустимые ключи в JSONB). Зафиксировать default значения, чтобы все клиенты видели одинаковое содержимое при пустом профиле.  
  **Лучший вариант:** Наложить строгие ограничения в Liquibase (`CHECK` + `NOT NULL`) и дублировать их Bean Validation-правилами, чтобы БД и приложение гарантировали консистентные значения.
- [x] Проработать модель ролей: таблицы `role`, `profile_role`, возможные `permissions` (admin, operator, user). Зафиксировать минимальные роли и связь с профилем, чтобы любой канал видел одинаковый access level.  
...
- [x] Экспонировать REST API `/api/profile`:
  ...
  **Лучший вариант:** Единый `ProfileController`, который резолвит `ProfileLookupKey` из path-параметров/headers и делегирует проверки `ProfileAccessService`, чтобы сохранить простые и предсказуемые REST-контракты.
- [x] Все REST-запросы, которые потребляют профиль, должны нести единый заголовок `X-Profile-Key` (формат `namespace:reference`). Добавить фильтр, который валидирует заголовок, строит `ProfileLookupKey` и прокидывает его во все сервисы (web/TG/CLI). Для обратной совместимости — заголовок обязателен во всех новых API.  
  **Лучший вариант:** Реализовать `HandlerMethodArgumentResolver`, который читает `X-Profile-Key`, валидирует его и предоставляет контроллерам типизированный `ProfileLookupKey`, сохраняя данные также в `SecurityContext` для фильтров.
- [x] Предусмотреть режим “без аутентификации”: backend принимает dev-only токен из `.env` (`PROFILE_BASIC_TOKEN`), передаваемый в заголовке `X-Profile-Auth`, и разрешает работу с профилями без OAuth/рольной модели (read/write в рамках одного токена). Введён централизованный guard, который отклоняет запросы без токена и полностью выключает дев-режим, когда `PROFILE_DEV_ENABLED=false`.  
  **Лучший вариант:** Подключить отдельный `AuthenticationProvider`, который проверяет dev-секрет, выдаёт `DevProfileAuthentication` и позволяет переиспользовать текущие security guard’ы/`@PreAuthorize`.
- [x] Ввести единый security guard для `/api/profile/**`, `/api/admin/**`, `/api/llm/**`, `/api/flows/**`, который валидирует `X-Profile-Key`, dev-токен и набор ролей до входа в контроллер, публикует структурные логи отказов и метрики (403/401), а также обеспечивает доступ к админ-эндпоинтам только для `admin`.
- [x] Нормализовать `user_identity` (provider/externalId) при сохранении/удалении: хранить их в стандартизованном регистре, предотвращать дубли с разным написанием и добавить regression-тесты на attach/detach.
- [x] Версионировать профиль (`version`/`updated_at`) и поддержать ETag/If-Match, чтобы одно и то же значение синхронно доходило до фронтенда, Telegram и CLI. Продумать реакции на конфликт (HTTP 409) и бэк-офисный merge.  
  **Лучший вариант:** Использовать optimistic locking (`@Version long version`) и выдавать `ETag: W/"<version>"`; при конфликте возвращать 409 и просить клиента перезагрузить профиль.
- [x] Встроить `UserProfileService` в `SyncChatService`, `StructuredSyncService`, `AgentInvocationService`: перед сборкой промпта подмешивать тон, язык, рабочие часы и анти-паттерны из профиля. Добавить тесты и фичефлаг `ENABLE_PROFILE_PROMPTS`, чтобы можно было откатить.  
  **Лучший вариант:** Собирать “persona snippet” на основе профиля, переводить его на нужную локаль и добавлять к system prompt перед вызовом модели.
- [x] Для `TelegramChatService`, CLI и веб-сокетов гарантировать генерацию `ProfileLookupKey` (`namespace=telegram`, `reference=telegramUserId` и т.д.), автосоздание профиля при первом сообщении и логирование событий (`profile_created`, `profile_updated`, `identity_attached`).  
  **Лучший вариант:** Каналы напрямую вызывают `UserProfileService.resolveOrCreate(...)`, который в одной транзакции создаёт профиль и запись lookup.
- [x] Реализовать событийную шину (`ProfileChangedEvent`) поверх Redis pub/sub: при обновлении профиля сервис публикует событие, все другие инстансы сбрасывают локальный кеш (Caffeine) и подтягивают свежие данные. Описать fallback (in-memory events) для dev-окружения.  
  **Лучший вариант:** Использовать Redis `PatternTopic profile:changed` с payload `{profileId, version}` и подписчиками, которые инвалидируют соответствующие записи; в dev окружениях fallback через `ApplicationEventPublisher`.
- [x] Подготовить `ProfileAdminController`/`RoleAssignmentService`: CRUD ролей, выдача/отзыв полномочий, аудит изменений. API должно работать поверх той же модели профиля, без дублирования сущностей.  
  **Лучший вариант:** REST API `/api/admin/roles` поверх `RoleAssignmentService`, который пишет в `profile_role` и `role_audit_log`.

### Frontend и UX
- [x] Добавить страницу “Personalization” в веб-UI (React/Vite): форма с display name, языком, привычками, anti-паттернами, рабочими часами, превью системного сообщения и кнопка “привязать Telegram/внешний провайдер (stub)”. Поддержать optimistic updates, спиннер кеширования и информирование о лимитах.  
  **Лучший вариант:** Выделить отдельный маршрут `/profile` и использовать React Query для загрузки/сохранения профиля по `namespace/reference`.
- [x] Показать активный профиль в основном чате (баннер или боковая панель). Дать возможность переключать канал (web/telegram) и видеть расхождения настроек.  
  **Лучший вариант:** Добавить верхний баннер с селектором канала (`<Select channel>`), который отображает текущий `X-Profile-Key`.
- [x] Хранить полученный `profile` в состоянии клиента (zustand/query cache) как единую сущность, а channel overrides отображать только при выборе конкретного канала. Добавить индикаторы “единый профиль” и подсказку, что изменения применятся ко всем платформам.  
  **Лучший вариант:** Использовать единый Zustand store (`useProfileStore`) с `profilesByChannel`, а экран чата подписывать только на активный канал.
- [x] Отрисовывать историю изменений (когда и откуда обновили профиль) для быстрых проверок — например, “обновлено через Telegram 10 минут назад”.  
  **Лучший вариант:** Добавить Timeline-компонент, который читает `/api/profile/{namespace}/{reference}/audit`.
- [x] До полноценного OAuth внедрить dev-only сессию (e.g. токен из `.env`) и документацию по связке Telegram ↔ веб через одноразовый код/QR.  
  **Лучший вариант:** Показать баннер “Dev session, not secure” и реализовать endpoint `/profile/dev-link`, выдающий одноразовый код для связывания Telegram.
- [x] Спроектировать базовую админ-панель (можно отдельным route) для управления ролями: таблица профилей, фильтры по namespace, действия “назначить роль”, “убрать роль”, просмотр истории. UI реализован, ограничения по роли `admin` работают и на backend (`/api/admin/roles`), и на frontend (раздел скрывается без прав).  
  **Лучший вариант:** Расширить существующий Flow Admin layout новой вкладкой “Roles” с `MUI DataGrid` и действиями назначения/отзыва роли.

### OAuth и провайдеры (подготовка)
- [x] Обновить `docs/architecture/personalization.md` и `docs/infra.md` с целевыми flow: redirect → code → token → `user_identity`, хранение `state` и `codeVerifier`, требования по шифрованию refresh/access токенов и TTL. Добавить диаграммы для веб-регистрации и Telegram deep-link, отметив, что VK OAuth реализуется в Wave 43.  
  **Лучший вариант:** Нарисовать sequence-diagrams в PlantUML и подробно расписать обращение с секретами в `docs/infra.md`.
- [x] Спроектировать интерфейсы `OAuthProviderRegistry`/`OAuthProviderClient`, описать конфигурацию и механизмы хранения токенов, но оставить фактические адаптеры (VK, GitHub, Google) на Wave 43.  
  **Лучший вариант:** Ввести SPI на базе `spring.factories`, где каждый провайдер реализует `OAuthProviderClient`.
- [x] Подготовить UX-спеки для подключения внешнего провайдера: кнопка/команда, одноразовый state, подтверждение и отображение результата. Реализация команд “привязать VK” переносится в Wave 43, но контракт API должен быть зафиксирован.  
  **Лучший вариант:** Web-first модалка с QR/ссылкой; после callback UI поллит `/api/profile/.../identities/status` и отображает результат привязки.
- [x] Зафиксировать требования к мониторингу/cron-джобам для проверки истечения OAuth-токенов, чтобы в Wave 43 достаточно было реализовать конкретные провайдеры без изменений архитектуры.
...
- [x] Запланировать отдельные сценарии для ролей: unit/интеграционные тесты `RoleAssignmentService`, e2e админ-панели, smoke для разграничения доступа (user не может вызвать admin API). Добавлен интеграционный тест guard’а, покрывающий требования.
...
- [x] Документировать и проверить dev-only режим: тесты подтверждают, что при включённом `PROFILE_BASIC_TOKEN` запросы без аутентификации принимаются, но роли/OAuth отключены; при отключенном флаге доступ запрещён. Добавлено описание в `docs/infra.md` и тесты на guard.
...
- [x] Добавить миграции/скрипты для выдачи роли admin существующим операторам, описать процесс предупреждения.

### Frontend / Admin UI
- [x] Создать отдельный раздел “Admin / Roles” в веб-UI: таблица профилей, фильтрация по namespace, действия “выдать роль”, “снять роль”, просмотр истории изменений (audit log).
...
- [ ] Подготовить `VkOAuthController`: `GET /auth/vk` редиректит на `https://id.vk.ru/authorize` по схеме из [auth-without-sdk](https://id.vk.com/about/business/go/docs/ru/vkid/latest/vk-id/connection/web/auth-without-sdk) с параметрами `response_type=code`, `client_id`, `scope=email,phone` (можно конфигурировать), `redirect_uri`, `state`, `code_challenge`, `code_challenge_method=S256`. Для безопасности реализовать PKCE (S256) и генерацию `state`/`device_id`, как описано в шаге 2 документа.  
  **Лучший вариант:** Собственный контроллер с `Spring WebClient`, который генерирует PKCE/`state`, хранит их в `profile_lookup` и редиректит пользователя на VK ID.
- [ ] Реализовать `POST /auth/vk/callback`: проверка `state`, обмен кода на токен через `https://id.vk.ru/oauth2/token` (шаг 4), сохранение `access_token`, `refresh_token`, `expires_in`, `user_id` в `user_identity`. Использовать шифрование/secret storage для токенов, как требует VK ID.  
  **Лучший вариант:** Хранить токены в таблице `user_identity_secret`, шифруя значения `pgcrypto`/`AES-GCM` с master key из Vault.
- [ ] Добавить сервис обновления токена: cron/queue вызывает `https://id.vk.ru/oauth2/token` с `grant_type=refresh_token` (шаг 6), логирует ошибки, инвалидирует токены по `oauth2/logout` (шаг 7) и дергает `user_info` для обогащения профиля (шаг 8). Все вызовы должны поддерживать retries и метрики (успех/ошибка, latency).  
  **Лучший вариант:** Реализовать Spring `@Scheduled` job с `RetryTemplate`, который читает `user_identity`, обновляет токены и логирует ошибки/метрики.
- [ ] Настроить дизайн кнопки входа в соответствии с гайдлайнами VK ID (шаг ПККЕ guidlines), прописать требования в UI. В REST API и frontend хранить `device_id`/`state` до завершения флоу.  
  **Лучший вариант:** Использовать официальный SVG/iframe от VK ID в модалке “Link VK account” и документировать требования в `docs/ux`.
- [ ] В Telegram/CLI реализовать команду “/link_vk”: отправляет пользователю auth-link, генерирует одноразовый `state`, после callback показывает статус привязки. После сохранения identity обновлять профиль и заносить событие аудита.  
  **Лучший вариант:** Telegram-бот отправляет inline кнопку “Открыть VK ID”, backend сохраняет `state` в Redis и после callback сообщает статус.
- [x] Подготовить задел для других провайдеров (GitHub, Google): интерфейсы `OAuthProviderClient`, общая таблица токенов, конфиги в `.env`.  
  **Лучший вариант:** Создать таблицу `oauth_token` с общими полями (`access_token`, `refresh_token`, `scopes`, `expires_at`) и связью с `user_identity`.
...
- [ ] Настроить bot (через @BotFather `/setdomain`) и домен, который будет использоваться в Telegram Login Widget, как описано в [официальной документации](https://core.telegram.org/widgets/login#linking-your-domain-to-the-bot). Бот и веб-UI работают на одном домене `https://ai.riabov.tech/`, поэтому этот URL должен задаваться через конфиг (`app.web.base-url`) и попадать в `/setdomain`, а также использоваться при генерации callback'ов.  
  **Лучший вариант:** Использовать единый домен `ai.riabov.tech` c reverse-proxy, отдающим `/.well-known/telegram-login`.
- [ ] Реализовать фронтенд-виджет: добавить на страницу профиля код `<script async src="https://telegram.org/js/telegram-widget.js?22" data-telegram-login="botname" data-size="large" data-auth-url="/auth/telegram/callback" data-request-access="write"></script>` (значения из конфига). Поддержать альтернативный режим `data-onauth`, если хотим обработку в JS.  
  **Лучший вариант:** Использовать классический `<script>`-виджет с редиректом и backend callback’ом.
- [ ] На backend создать `TelegramAuthController`: принимает payload (`id`, `first_name`, `last_name`, `username`, `photo_url`, `auth_date`, `hash`), строит `data_check_string` (ключи по алфавиту, `key=value` c `\n`) и сравнивает `hash` = `hex(HMAC_SHA256(data_check_string, SHA256(bot_token)))`. При успехе маппит Telegram `id` к профилю, выдает одноразовый код для привязки.  
  **Лучший вариант:** REST endpoint `/auth/telegram/callback`, который сразу валидирует подпись и создаёт/обновляет `user_identity`.
- [ ] Добавить защиту от replays: проверять `auth_date` (например, не старше 1 минуты), хранить использованные `hash`/`auth_date` в Redis, логировать попытки. Прописать обработку ошибки в UI.  
  **Лучший вариант:** Использовать Redis `SETNX used_hash:<hash>` с TTL 60 секунд.
- [ ] Обновить `TelegramChatService`: если пользователь прошёл login-widget, подтягивать расширенные поля (avatar, username) и записывать в `user_identity`. При каждом апдейте чат-сервис должен проверять роли из профиля (admin/operator) и скрывать команды, которые недоступны.  
  **Лучший вариант:** При каждом апдейте чат-бот вызывает `UserProfileService.resolveByIdentity("telegram", telegramId)` и кэширует роли в `TelegramChatState`.
...
- [ ] Обновить `docs/architecture/personalization.md`, `docs/infra.md`, `docs/processes.md` разделами про RBAC, админку и VK OAuth.  
  **Лучший вариант:** Добавить большой раздел в `personalization.md` с диаграммой компонентов и описанием API/кеша, а остальные документы сослать на него.
- [ ] Написать тесты: unit для RBAC, интеграционные для `/api/admin/roles`, e2e для админ-панели и Telegram команды “Привязать VK”, smoke для истечения токенов.  
  **Лучший вариант:** Писать тесты на Java: `@DataJpaTest` для ролей, `@SpringBootTest` для профиля и Playwright/Telegram Bot API mock для e2e.
- [ ] Добавить end-to-end сценарий “VK OAuth + выдача роли”: пользователь проходит VK авторизацию, получает запись в `user_identity`, оператор через админку назначает роль, система проверяет доступ в чате/вебе и наличие токена в фоне. Обновлять документацию по результатам прогона.  
  **Лучший вариант:** Написать Playwright-тест, который через mock VK OAuth и UI-админку выдаёт роль и проверяет доступ.
...
### Архитектура и конфигурация
- [x] Перенести настройки OpenAI в `application-coding.yaml`: включить `spring.ai.openai.enabled=true`, использовать shared env из RAG, добавить override для coding-профиля, чтобы активировать GPT-4o Mini без дублирования переменных.
- [x] Расширить `CodingAssistantProperties` новой секцией `openai` (enabled, temperature, maxTokens, лимиты по размеру ответа) и описать, что при пустых значениях берутся общие настройки RAG.
- [x] Зафиксировать формат JSON-ответа модели: массив операций `{path, action, languageHint, contents, insertBefore}` + сводка и предупреждения. Документировать ограничения (строгий UTF-8, максимум 2000 строк на файл, запрет бинарных payload).

### Backend (Coding MCP)
- [x] Реализовать сервис `WorkspaceArtifactGenerator`, который принимает workspace/instructions/targetPaths, вызывает OpenAI через `ChatClient`/`OpenAiChatModel`, валидирует JSON и превращает его в список операций записи.
- [x] Обновить `WorkspaceFileService`: добавить методы записи (создание, overwrite, append) с проверкой, что путь остаётся в workspace, не входит во `forbiddenPaths` и не пересекается с существующими каталогами.
- [x] После применения операций собирать `git status`/`git diff` в workspace, формировать summary и `CodingPatch` (опционально) так же, как это делает текущий генератор, чтобы downstream-инструменты (review/apply) могли работать без изменений.
- [x] Добавить MCP tool `coding.generate_artifact`: описание входа (`workspaceId`, `instructions`, `operationsLimit`, `targetPaths`), возврат созданных файлов, diff и рекомендаций по дальнейшим шагам (например, dry-run через `coding.apply_patch_preview`).
...
Цель: заменить эвристические AST-метаданные и плоский call graph на полноценный граф кода, построенный на Tree-sitter и графовой БД (Neo4j/JanusGraph). MCP/LLM должны получать такую же картину проекта, как IDE: связи “файл → класс → метод”, вызовы, имплементации, пути от контроллеров до БД и т.д. Используем Neo4j как основной движок (Testcontainers + Aura-compatible деплой), tree-sitter подключаем через JavaCPP/JNI, синхронизацию графа делаем асинхронной, но с гарантией, что `graphReadyAt` выставляется только после успешной загрузки всех узлов/ребер.

### Выбранный вариант реализации (best effort)
- Runtime: Java binding `ch.usi.si.seart:java-tree-sitter` с загрузкой собранных грамматик из `treesitter/<os>/<arch>/libtree-sitter-*.{so,dylib}`. `TreeSitterLibraryLoader` копирует библиотеки во временный каталог и отдаёт `Path` для `Language.fromPath`, с graceful fallback на эвристику при отсутствии артефактов. В bootJar пакуются грамматики и `libjava-tree-sitter.*` (текущий артефакт — x86_64; arm64 требует сборки вручную или перехода на `io.github.tree-sitter:jtreesitter`).
- AST: `TreeSitterParser#parseNative` обходит дерево для Java/Kotlin/TS/JS/Python/Go, строит нормализованный `SymbolNode`: FQN `package.Class#method(argTypes)`, kind/visibility/signature/docstring, span, imports, флаги test. Рёбра: `CONTAINS`, `CALLS` (по call_expression), `IMPLEMENTS/EXTENDS` (bases/interfaces), `READS_FIELD` (member access), `USES_TYPE` (type_reference/annotation). `AST_VERSION` поднят до 2.
- Граф: `GraphSyncService` пишет в Neo4j `CALLS/IMPLEMENTS/READS_FIELD/USES_TYPE` с chunkHash/chunkIndex. Старый SQL граф остаётся флагом `legacy-table-enabled` для отката. Фичи готовности `graphReady`/`graphSchemaVersion` отражаются в `repo.rag_index_status`.
- Поиск/UI: postprocessing добавляет “graph lens” в `matches[].metadata.graph_neighbors/graph_path`, модуль `graph.lens` в `appliedModules`. MCP инструменты `repo.code_graph_*` используют Neo4j, при деградации падают на PARENT/LINEAR с предупреждением.
- Тесты/CI: Testcontainers Neo4j e2e (index → graph sync → code_graph_neighbors/path), unit на нативный парсер по языкам (fixtures с наследованием/интерфейсами/перегрузками/docstring). CI шаг `treeSitterVerify` + нативный smoke; отдельный Neo4j smoke “workspace fetch → index → search with graph lens”.

### Tree-sitter и AST слой
- [x] Реализовать `TreeSitterParser`/`AstNode` обёртку вокруг собраных грамматик (`treesitter/*`), поддерживающую Java/Kotlin/TS/JS/Python/Go (минимум) и готовую к расширению. Прогонять парсинг внутри `AstFileContextFactory` вместо `HeuristicAstExtractor`. Управление parser-ами возложить на `TreeSitterAnalyzer`: кешировать лес на файл, реиспользовать для chunker-а и graph-sync.
- [x] Стандартизировать FQN в формате `package.Class#method(argTypes)`/`package.Class` для типов. Построить нормализованный DTO `SymbolNode` (uuid, fqn, kind, modifiers, signature, docstring, span, imports, parentId) и набор edge DTO (`CONTAINS`, `CALLS`, `IMPLEMENTS`, `READS_FIELD`, `USES_TYPE`). Обновить `AstSymbolMetadata`, `Chunk` и semantic chunker, чтобы они опирались на новую структуру.
...
- [ ] Перейти с `ch.usi.si.seart:java-tree-sitter` на официальный `io.github.tree-sitter:java-tree-sitter`:
  ...
  - [ ] Подключить Java bindings грамматик из сабмодулей (`bindings/java`) через `sourceSets`; собрать реестр языков `Language(TreeSitter<Lang>.language())` вместо статических `Language.JAVA`.
  - [ ] Настроить загрузку нативок через `NativeLibraryLookup` под layout `treesitter/<os>/<arch>/lib{tree-sitter,tree-sitter-<lang>}` (arm64/x86_64), использовать собранные библиотеки из сабмодулей.
  - [ ] Обновить `TreeSitterParser` под jtreesitter API (`Parser#setLanguage`, `parseString`, явное закрытие ресурсов) и вернуть нативный разбор; эвристический fallback оставить для отсутствующих грамматик.
  ...
  - [ ] Добавить query-слой (SCM-шаблоны per язык) для symbols/imports/calls и интегрировать его в парсер, чтобы строить edges `CALLS/IMPLEMENTS/USES` без эвристик.
...
- [x] Настроить инкрементальные апдейты: при удалении файла чистить узлы/ребра, при изменении — пересобирать только затронутые символы. Idempotent операции + логирование размера батчей, времени sync, числа добавленных/удалённых узлов. Добавить конфиг флага `github.rag.graph.legacy-table-enabled` для отключения старого `RepoRagSymbolGraphWriter`.

### MCP инструменты и использование графа
- [x] Добавить инструменты `repo.code_graph_neighbors`, `repo.code_graph_path`, `repo.code_graph_definition`: вход — `namespace`, `symbolFqn`, `direction`, `relation`, `depth/limit`. Выход — JSON `{nodes:[{id,fqn,file,line,kind}], edges:[{from,to,relation}]}` + подсказки для UI. Контракты описать в `docs/tools.md`.
- [x] Переписать `NeighborChunkDocumentPostProcessor` и `RepoRagSymbolService` на графовую БД: вместо таблицы `repo_rag_symbol_graph` исполнять подготовленные Cypher/GQL запросы и возвращать точные соседи. Фолбэком остаётся LINEAR/PARENT при `graphReady=false` или ошибке Neo4j; предупреждение писать в `warnings[]`.
- [ ] В генерации ответов добавлять “graph lens”: `RepoRagSearchService`/`CodeAwareDocumentPostProcessor` вытягивают для каждого top chunk список соседей и краткий путь до связанных символов, пишут их в `matches[].metadata.graph_neighbors`/`graph_path` и `appliedModules+=graph.lens`. UI должен отображать эти подсказки (описать в `docs/guides/mcp-operators.md`).
...
- [ ] Вернуть реальный нативный парсинг через jtreesitter: `TreeSitterParser.parseNative` с queries per язык, loader на `NativeLibraryLookup` для `libtree-sitter` + языковых либ из `treesitter/<os>/<arch>`, подключить `bindings/java` сабмодулей в sourceSets, оставить health-fallback.
- [ ] Исправить построение FQN/иерархии: стек контейнеров, сигнатуры с типами, docstring/visibility/test-флаг, исключить подвешивание функций на последний класс.
- [ ] Нормализовать рёбра графа: `CALLS/IMPLEMENTS/READS_FIELD/USES_TYPE` по полным FQN с разрешением импортов, без stub-нод; чистить стубы в Neo4j и при реиндексе после AST_VERSION=2.
- [ ] Разморозить и дополнить тесты: обновить fixtures mini-repos (наследование/интерфейсы/перегрузки/docstring), включить `TreeSitterParserNativeSmokeTest`/`RepoRagNativeGraphSmokeTest`, проверить FQN/edges.
- [ ] Привести `AstFileContext.create`/`AstFileContextFactory` к рабочему состоянию (убрать заглушки/дубли импорта).

...
- **Структурированные ответы**: обрабатываем payload аналогично фронтенду.
  1. Markdown карточка: summary + список деталей в одном сообщении.
  2. Split-режим: краткий summary сразу, детали по inline-кнопке отдельным сообщением.
  3. Вложение: summary в тексте, JSON/таблица — отдельным вложением/файлом для power users.
...
- [ ] Описать, какие элементы UI FE переносятся в Telegram напрямую, а какие требуют адаптации (например, сегменты моделей, sampling sliders, вывод structured card), и какие данные показываем в инлайн-кнопках/сообщениях; итогом должно быть зафиксированное решение по доставке финального ответа и отображению structured payload. Возможные подходы:
    1. **Зеркалирование UI** — повторить карточки и сегменты почти без изменений (больше inline-кнопок и reply markup).
    2. **Упрощённый стек** — оставить выбор провайдера/модели и sampling в отдельных меню, usage/стоимость показывать отдельным сообщением по кнопке.
    3. **Контекстные действия** — минимальный главный поток (сообщения + usage), управление переносится в inline-меню «Настройки»/«Инструменты».
...
-     *Варианты onboarding/команд*: 
      - интро-сообщение с каруселью кнопок («Выбери модель», «Запусти research», «Справка»);
      - пошаговый мастер (первое сообщение — выбор провайдера, второе — режим, третье — инструменты);
      - минималистичный `/start` + подсказки в inline-меню, все сценарии доступны командами `/mode`, `/tools`, `/new`.
...
- [ ] Реализовать сервис бота в backend: обработка апдейтов, хранение состояния пользователя (chatId → текущая сессия, выбранная модель, sampling overrides, подключённые инструменты) в памяти процесса бота с интерфейсом для последующей замены на Redis.
...
- [ ] Выбрать стратегию доставки финального ответа (sync уже принят, при необходимости описать fallback-фасад) и закрепить реализацию в адаптере Telegram.
-     *Варианты хранения состояния и масштабирования*:
      1. In-memory store с snapshot-дампом, sticky sessions на уровне webhook.
      2. Redis (single node) c TTL на параметры и распределением нагрузки.
      3. PostgreSQL/Key-Value таблица с историей настроек для аудита.
...
- [ ] Реализовать управление режимами Streaming/Sync/Structured и sampling-параметрами (temperature, topP, maxTokens) через inline-кнопки/команды, мапируя их на `buildChatPayload` (`frontend/src/pages/LLMChat.tsx:2520`, `frontend/src/pages/LLMChat.tsx:2644`), и сохранять текущие настройки в in-memory сторе.

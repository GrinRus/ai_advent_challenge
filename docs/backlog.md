...
### Backend (backend-mcp)
- [x] Ввести интерфейс `PatchGenerator` и реализацию `ClaudeCliPatchGenerator`, которая собирает JSON-команду (`instructions`, `targetPaths`, `forbiddenPaths`, `contextSnippets`) и запускает `claude -p <payload> --output-format json --no-ansi` внутри workspace. Проверить, что возвращаемый JSON содержит `summary`, `diff` (unified), `files[]`, `risks[]`, `usage.promptTokens/completionTokens`; навесить валидации лимитов (`coding.max-*`). → `backend-mcp/src/main/java/com/aiadvent/mcp/backend/coding/*`
- [x] Добавить `ClaudeCliService`: поиск бинаря по `CLAUDE_CODE_BIN` или `$PATH`, валидация (`claude --version`), прокидка `ZHIPU_API_KEY`, конфигурация таймаутов (`CLAUDE_CODE_TIMEOUT`), запуск через `ProcessBuilder` с отдельными потоками stdout/stderr и редактированием ANSI (флаги `--no-ansi`). Вернуть типизированный ответ/ошибку (timeout, non-zero exit, stderr warning). → `backend-mcp/src/main/java/com/aiadvent/mcp/backend/coding`

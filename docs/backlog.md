...
- [ ] Зафиксировать архитектурное решение: описать целевую схему подключения Docker MCP к демону через docker-java/Testcontainers Foundation, включая требования к лог-стримингу, обработке статусов и тайм-аутам.
- [ ] Определить требования безопасности и совместимости: поддержка unix-socket/TCP, TLS для DinD, ограничения по ресурсам, политика очистки контейнеров.
- [ ] Подготовить план отката к CLI-режиму (описать feature flag/профиль и критерии переключения).

### Backend (docker-runner сервис)
- [ ] Добавить зависимости `docker-java` или `testcontainers-foundation` в `backend-mcp/build.gradle`, вынести версию в версионный каталог.
- [ ] Реализовать конфигурацию клиента (`DockerClientConfig`, поддержка env-переменных из `DockerRunnerProperties` для socket/TLS/credentials).
- [ ] Переписать `DockerRunnerService` на использование docker-java API: создание контейнера, настройка `HostConfig`, запуск, ожидание завершения, принудительное завершение по тайм-ауту.
- [ ] Реализовать live-стрим stdout/stderr через `ResultCallback` с публикацией чанков в текущую модель ответа и ограничением `maxLogBytes`.
- [ ] Обеспечить корректную уборку: `removeContainerCmd`, удаление orphan-контейнеров при ошибках, закрытие callback’ов и клиента.
- [ ] Расширить `DockerRunnerProperties` новыми настройками (TLS cert path, режим лог-стрима, стратегия удаления контейнеров) и задокументировать в `application-docker.yaml`.

### Инфраструктура и окружения
- [ ] Удалить необходимость установки `docker.io` в `backend-mcp/Dockerfile`, оставить только необходимые утилиты.
- [ ] Проверить, что контейнеру достаточно сокета/переменных для работы docker-java (unix socket permissions, TLS каталоги).
- [ ] Обновить `docker-compose.yml`: зафиксировать новые env-переменные (`DOCKER_RUNNER_DOCKER_HOST`, `DOCKER_RUNNER_TLS_VERIFY`, `DOCKER_RUNNER_CERT_PATH` и т.д.).

### Тестирование и наблюдаемость
- [ ] Написать интеграционный тест, поднимающий временный docker daemon (через Testcontainers) и проверяющий полный цикл запуска Gradle-задачи с логами и тайм-аутом.
- [ ] Добавить unit-тесты на маппинг `DockerRunnerProperties` → `HostConfig`/`CreateContainerCmd`.
- [ ] Настроить метрики/логи для новых операций: время создания контейнера, количество очисток, ошибки API клиентов.
- [ ] Обновить GitHub Actions пайплайн для прогонов новых интеграционных тестов (добавить требуемые сервисы/права).

### Документация и релиз
- [ ] Обновить `docs/infra.md` и `README.md`: новые переменные окружения, схема подключения, инструкции по отладке.
- [ ] Подготовить release notes для Wave 22.1: изменения в API, миграция, шаги для DevOps.
- [ ] Провести cutover: включить docker-java режим по feature flag, мониторить метрики, закрыть тикет после успешной проверки.
...
- [ ] Зафиксировать фичи фронтенд-чата: выбор провайдера/модели с сегментами, переключение режимов Streaming/Sync/Structured, overrides sampling-параметров, выбор MCP-серверов, отображение usage/стоимости, кнопки «Новый диалог»/остановки стрима, ссылки на Flow UI (`frontend/src/pages/LLMChat.tsx:2380`, `frontend/src/pages/LLMChat.tsx:2403`, `frontend/src/pages/LLMChat.tsx:2520`, `frontend/src/pages/LLMChat.tsx:2536`, `frontend/src/pages/LLMChat.tsx:2960`).
- [ ] Описать, какие элементы UI FE переносятся в Telegram напрямую, а какие требуют адаптации (например, сегменты моделей, sampling sliders, вывод structured card) и какие данные показываем в инлайн-кнопках/сообщениях.
- [ ] Описать пользовательские сценарии Telegram-бота: onboarding, выбор модели/инструментов, отправка сообщений, получение результатов, fallback на ошибки.
- [ ] Согласовать UX-команды и команды бота (`/start`, переключатели моделей, help) и обновить спецификацию.
- [ ] Спроектировать inline-меню выбора моделей, кнопку «Начать диалог заново» и UX для мультимодальных сообщений (текст + голос).

### Backend и интеграция с Telegram API
- [ ] Настроить Telegram Bot API: регистрация бота, конфигурация токена и режима (webhook/polling) через переменные окружения.
- [ ] Реализовать сервис бота в backend: обработка апдейтов, хранение состояния пользователя (chatId → текущая сессия, выбранная модель и инструменты).
- [ ] Интегрировать вызовы существующего чат-сервиса: проксировать запросы, обеспечивать поддержку инструментов и стриминга ответов.
- [ ] Добавить обработку edge-case'ов: rate limit, время ожидания, ошибки API LLM/инструментов, логирование и трейсинг для диалогов.
- [ ] Реализовать webhook-приёмник апдейтов Telegram, обеспечить валидацию подписи и обработку повторных доставок.
- [ ] Добавить inline-кнопки для выбора моделей/инструментов и команду сброса контекста, синхронизировать состояние с backend-сессией.
- [ ] Реализовать управление режимами Streaming/Sync/Structured и sampling-параметрами (temperature, topP, maxTokens) через inline-кнопки/команды, мапируя их на `buildChatPayload` (`frontend/src/pages/LLMChat.tsx:2520`, `frontend/src/pages/LLMChat.tsx:2644`).
- [ ] Формировать ответы Telegram с метаданными (провайдер, модель, инструменты, usage/cost) и снабдить их действиями «Показать детали»/«Повторить запрос», чтобы сохранить прозрачность UI (`frontend/src/pages/LLMChat.tsx:2403`, `frontend/src/pages/LLMChat.tsx:2800`).
- [ ] Добавить возможность остановки стрима и повторной отправки запроса из Telegram (callback-кнопки/команды), поддерживая обновление sessionId и history (`frontend/src/pages/LLMChat.tsx:2982`).
- [ ] Поддержать голосовые сообщения: конвертация через STT, выбор модели обработки, возврат текста/ответа пользователю.

### Инфраструктура и тестирование
- [ ] Добавить конфиг для webhook/polling режима в `docker-compose` и CI, обеспечить безопасное хранение токенов.
- [ ] Настроить интеграционные тесты/контуры, эмулирующие Telegram апдейты и проверяющие end-to-end сценарии (выбор модели, инструменты, ответы).
- [ ] Обновить мониторинг: метрики активности бота, доля успешных ответов, задержки на инструменты, оповещения при деградации.
- [ ] Покрыть unit/contract-тестами callback-обработчики inline-кнопок (переключение провайдера/модели, режимов, остановка стрима, повтор, голосовые сообщения).

### Документация и релиз
- [ ] Обновить `docs/infra.md`, `README.md` и product-гайды: настройка бота, управление токенами, сценарии использования, ограничения.
- [ ] Подготовить внутреннее объявление релиза Wave 27: чек-лист запуска, инструкции для тестирования, обратная связь от первых пользователей.

...
- [x] Вернуть реальный нативный парсинг через jtreesitter: `TreeSitterParser.parseNative` с использованием jtreesitter, базовый обход дерева и сбор вызовов/наследования/чтений из AST, health-fallback при ошибках.
- [ ] Добавить queries per язык и loader на `NativeLibraryLookup` для `libtree-sitter` + языковых либ из `treesitter/<os>/<arch>`, подключить `bindings/java` сабмодулей в sourceSets.

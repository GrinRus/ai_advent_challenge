...
- **Логика**
  ...
  - Перед вызовом LLM собираем контекст (ограничение по размеру); используется Claude Code CLI поверх GLM (`https://api.z.ai/api/anthropic`, модели `GLM-4.5/GLM-4.6`). CLI запускается как подпроцесс, выводит JSON `{summary,diff,files,risks,usage,requiresManualReview}`; при отключении feature-flag используется fallback Patch Plan.

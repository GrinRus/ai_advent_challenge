...
1. В `.env` или переменных окружения задайте:
   ...
   ```
2. Перезапустите backend. Пока флаг включен, все запросы к `/api/profile/*`, `/api/llm/*`, `/api/flows/*` и `/api/admin/*` обязаны передавать заголовок `X-Profile-Auth: dev-profile-token` и будут приняты без OAuth.
3. Клиент должен по-прежнему передавать `X-Profile-Key`/`X-Profile-Channel`; единственное отличие — токен заменяет настоящую сессию. Центральный guard отклоняет запросы без заголовка или с неверным токеном.
...
6. Обязательно выключайте режим (`PROFILE_DEV_ENABLED=false`) перед публикацией или совместной разработкой: backend начнёт возвращать `403` для всех защищённых эндпоинтов, пока не будет включена полноценная аутентификация.
7. Админ-панель (`/admin/roles`) и REST `/api/admin/**` доступны только профилям с ролью `admin`. UI скрывает раздел без права, а backend возвращает `403`, если guard не находит роль в активном профиле.

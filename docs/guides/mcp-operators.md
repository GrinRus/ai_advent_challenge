...
4. **Разбор ответов:** `instructions` — готовый system prompt (учитывает `generationLocale` и шаблон). `augmentedPrompt` теперь отражает исходный output `ContextualQueryAugmenter` (запрос + контекст, как он пойдёт в LLM), поэтому можно буквально увидеть, что будет отправлено модели до подстановки шаблонов. `matches[].metadata.generatedBySubQuery` помогает понять, какой подзапрос вернул чанк; дополнительно доступны `parent_symbol`, `span_hash`, `overlap_lines`, `neighborOfSpanHash` (указывает, вокруг какого исходного span был расширен ответ).
5. **Новый контракт (Wave 36):** инструменты `repo.rag_search` и `repo.rag_search_global` принимают только `repoOwner?`, `repoName?` (только для repo-скоупа), `rawQuery`, `profile`, `conversationContext` (`history[] + previousAssistantReply`). Любые дополнительные поля приводят к ошибке `Unsupported parameter`. Чтобы поменять стратегию поиска, указываем `profile` (`conservative`, `balanced`, `aggressive`, либо свой, определённый в `github.rag.parameter-profiles`). Если поле не задано, MCP автоматически использует `github.rag.default-profile` и добавляет предупреждение во `warnings[]`.

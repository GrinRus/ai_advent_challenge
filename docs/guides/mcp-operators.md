...
4. **Расширенный поиск (`repo.rag_search` v4):**
   ...
   - `appliedModules` отражает цепочку модулей (semantic chunking → compression → rewrite → translation → multi-query → post.code-aware → post.heuristic-rerank → context-budget → llm-компрессор). Если модуль отсутствует, он отключён или не внёс изменений.
   - Конфигурация:

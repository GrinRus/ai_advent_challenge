package com.aiadvent.backend.mcp.agentops;

import com.aiadvent.backend.mcp.config.AgentOpsBackendProperties;
import com.aiadvent.backend.mcp.config.ReactorClientHttpConnectorBuilder;
import java.time.Duration;
import org.springframework.context.annotation.Bean;
import org.springframework.context.annotation.Configuration;
import org.springframework.http.HttpHeaders;
import org.springframework.web.reactive.function.client.WebClient;

@Configuration
class AgentOpsClientConfiguration {

  @Bean
  WebClient agentOpsWebClient(AgentOpsBackendProperties properties) {
    WebClient.Builder builder =
        WebClient.builder()
            .baseUrl(properties.getBaseUrl())
            .defaultHeader(HttpHeaders.ACCEPT, "application/json")
            .defaultHeader(HttpHeaders.CONTENT_TYPE, "application/json")
            .clientConnector(
                new ReactorClientHttpConnectorBuilder()
                    .connectTimeout(Duration.ofSeconds(10))
                    .readTimeout(Duration.ofSeconds(30))
                    .build());

    if (properties.getApiToken() != null && !properties.getApiToken().isBlank()) {
      builder.defaultHeader(
          HttpHeaders.AUTHORIZATION, "Bearer " + properties.getApiToken().trim());
    }

    return builder.build();
  }
}

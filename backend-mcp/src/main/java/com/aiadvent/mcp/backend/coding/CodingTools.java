...
class CodingTools {
  ...
  @Tool(
      ...
      description =
          ...
          {"workspaceId": "...", "instructions": "...", "targetPaths": ["src/App.java"], "forbiddenPaths": [...],
          "contextFiles": [{"path": "README.md", "maxBytes": 20480}]}. Поля workspaceId и instructions обязательны,
          инструкции должны быть непустыми. Пути указываются относительно корня workspace и не могут ссылаться на
          каталоги (contextFiles читает только файлы). targetPaths помогает сфокусировать генерацию, forbiddenPaths
          блокирует затрагивание файлов. Чтобы создать новый файл, добавьте путь в targetPaths и опишите содержимое во
          instructions — diff будет включать блок с new file."""
  ...
  @Tool(
      ...
      description =
          ...
          "patchId": "...", "focus": ["risks","tests"]}. Обязательны workspaceId и patchId. focus — необязательный список
          режимов анализа (risks, tests, migration); регистр нечувствителен. В ответе возвращаются findings, советы по
          тестам, nextSteps и аннотации, которые можно показывать оператору перед публикацией."""
  ...
  @Tool(
      ...
      description =
          ...
          "dryRun": true, "timeout": "PT15M"}. workspaceId и patchId обязательны. По умолчанию dryRun=true, timeout
          подразумевает ISO 8601 duration. Команды поддерживают только gradle/./gradlew. После успешного прогонов diff
          откатывается, а метаданные dry-run обновляются в реестре."""
  ...
  @Tool(
      ...
      description =
          ...
          patchId, статус, requiresManualReview, наличие dry-run и прочие метрики, что удобно для отображения очереди
          задач оператора или повторного запуска review/apply."""
  ...
  @Tool(
      ...
      description =
          """
          Удаляет патч из реестра и освобождает связанные ресурсы. Ожидает {"workspaceId": "...", "patchId": "..."} —
          оба поля обязательны. Используйте после успешной публикации или если патч более не нужен, чтобы очистить
          реестр и прекратить дальнейшие dry-run/review для него."""

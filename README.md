...
- Добавлен инструмент `repo.rag_search_simple` — достаточно передать только `rawQuery`, MCP повторно использует репозиторий из последней `github.repository_fetch` и ожидает готовности индекса.
